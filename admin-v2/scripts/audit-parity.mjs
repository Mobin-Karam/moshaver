...
const failures = Object.entries(contracts).flatMap(([area, tokens]) =>
  tokens.filter((token) => !source.includes(token)).map((token) => `${area}: ${token}`),
);
console.log(
  `Parity contracts: ${Object.keys(contracts).length} areas, ${Object.values(contracts).flat().length} required integrations.`,
);
if (failures.length) {
