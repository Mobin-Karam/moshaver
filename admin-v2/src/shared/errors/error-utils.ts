...
export function classifyAppError(error: unknown): AppErrorDetails {
  ...
  if (status === 403) {
    return {
      ...
      description: "نقش یا زمینه کاری فعال شما اجازه استفاده از این بخش را ندارد.",
    };
  }
  if (/chunk|dynamically imported module|failed to fetch module/.test(message)) {
    return {
      kind: "chunk",
      status,
      title: "نسخه جدید صفحه آماده است",
      description: "بخشی از برنامه به‌روز شده است. صفحه را دوباره بارگذاری کنید.",
    };
  }
  ...
  return {
    kind: "unknown",
    status,
    title: "مشکلی در بارگذاری صفحه رخ داد",
    description: "اطلاعات شما تغییری نکرده است. دوباره تلاش کنید یا به میز کار برگردید.",
  };
}
