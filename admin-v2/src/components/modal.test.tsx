...
function Harness({ resolved }: { resolved: (value: boolean) => void }) {
  ...
  return <><button onClick={() => modal.open({ title: "ویرایش دانش‌آموز", content: <input aria-label="نام" />, size: "lg" })}>open</button><button onClick={() => void modal.confirm({ title: "حذف شود؟", description: "این عملیات قابل بازگشت نیست.", tone: "danger", confirmLabel: "حذف" }).then(resolved)}>confirm</button></>;
}
...
describe("global modal", () => {
  it("renders through the provider and closes with Escape", async () => {
    render(<ModalProvider><Harness resolved={() => undefined} /></ModalProvider>);
  ...
  it("resolves reusable confirmations", async () => {
    ...
    render(<ModalProvider><Harness resolved={resolved} /></ModalProvider>);
