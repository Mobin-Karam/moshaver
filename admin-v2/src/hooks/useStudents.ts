...
export function useStudents() {
  ...
  const query = useQuery({ queryKey: ["students"], queryFn: () => api.get<Student[] | { items: Student[] }>("/admin/students?limit=100") });
  const students = Array.isArray(query.data) ? query.data : query.data?.items ?? [];
