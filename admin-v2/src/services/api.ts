type ApiEnvelope<T> = { ok: true; data: T } | { ok: false; error?: { message?: string; code?: string; details?: unknown } };
...
async function refreshCsrf() {
  const me = await request<{ csrfToken?: string }>("GET", "/auth/me", undefined, { noCsrfRetry: true, suppressAuthFailure: true });
  if (me.csrfToken) setCsrf(me.csrfToken);
  return me;
}

export async function request<T>(method: string, path: string, body?: unknown, options: { timeoutMs?: number; noCsrfRetry?: boolean; suppressAuthFailure?: boolean } = {}): Promise<T> {
  ...
  try {
    ...
    const apiError = new ApiError(response.status, err?.message || "خطای سرور", err?.code || "HTTP_ERROR", err?.details ?? null);
    if (response.status === 403 && apiError.code === "CSRF" && isMutating(method) && !options.noCsrfRetry) {
      await refreshCsrf();
