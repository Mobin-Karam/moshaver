...
const destinations = [
  ["/admin/learning", "یادگیری", "آیتم‌ها و وضعیت یادگیری", BookOpenCheck,"learning.read"],
...
export function StudentAdminAccess({ selectedId }: { selectedId: string }) {
  ...
  return <section className="grid gap-3" aria-labelledby="student-access-heading">
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      {destinations.filter(([, , , ,capability])=>!capability||auth.can(capability)).map(([path, label, description, Icon]) => <Link key={path} className="group flex min-h-16 items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 text-right transition hover:border-slate-300 hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand dark:border-slate-800 dark:bg-slate-900 dark:hover:border-slate-700 dark:hover:bg-slate-800" to={`${path}?${query}`}>
