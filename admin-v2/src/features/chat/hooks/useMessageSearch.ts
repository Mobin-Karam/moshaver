...
export function useMessageSearch(items: ChatMessage[], conversationId?: string) {
  ...
  const matches = useMemo(() => {
    ...
    if (!needle) return [];
    return items
      .filter((message) => !message.deletedAt && normalize(message.text || "").includes(needle))
  ...
  useEffect(() => {
    setIndex((current) => (matches.length ? Math.min(current, matches.length - 1) : 0));
  }, [matches.length]);
