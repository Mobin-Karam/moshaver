...
describe("chat helpers", () => {
  ...
  it("merges cursor pages chronologically without duplicates", () => {
    const latest: MessagePage = {
      messages: [
        { id: "2", text: "دو", senderRole: "admin" },
        { id: "3", text: "سه", senderRole: "student" },
      ],
    };
    const older: MessagePage = {
      messages: [
        { id: "1", text: "یک", senderRole: "student" },
        { id: "2", text: "دو", senderRole: "admin" },
      ],
    };
    expect(
      mergeMessagePages({ pages: [latest, older], pageParams: ["", "2"] }).map((item) => item.id),
    ).toEqual(["1", "2", "3"]);
  });
  ...
  it("hides group actions disabled by member permissions", () => {
    const group = {
      ...
      permissions: {
        members_can_react: 0,
        members_can_edit_own_messages: 0,
        members_can_delete_own_messages: 0,
      },
