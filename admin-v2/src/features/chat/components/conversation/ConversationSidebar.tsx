...
export function ConversationSidebar({ visible, items, activeId, search, filter, sort, favoriteIds, drafts, total, unread, loading, error, fetching, hasMore, fetchingMore, onSearch, onFilter, onSort, onSelect, onToggleFavorite, onRetry, onMore, onGroupCreated }: {
  visible: boolean; items: Conversation[]; activeId?: string; search: string;
  filter: ConversationFilter; sort: ConversationSort; favoriteIds: Set<string>; drafts: Record<string, string>; total: number; unread: number; loading: boolean;
  error: boolean; fetching: boolean; hasMore: boolean; fetchingMore: boolean;
  onSearch: (value: string) => void; onFilter: (value: ConversationFilter) => void; onSort: (value: ConversationSort) => void;
  onSelect: (item: Conversation) => void; onToggleFavorite: (id: string) => void; onRetry: () => void; onMore: () => void;
  ...
  return <Card className={`${visible ? "flex" : "hidden lg:flex"} min-h-0 flex-col overflow-hidden border-slate-200/90 p-0 shadow-[0_12px_35px_rgba(31,49,46,0.06)]`}>
    ...
    <div className="min-h-0 flex-1 overflow-auto">
      {loading ? <ConversationSkeleton /> : error ? <EmptyState title="دریافت گفتگوها ناموفق بود." action={<Button variant="soft" onClick={onRetry}>تلاش دوباره</Button>} /> : <ConversationList items={items} activeId={activeId} favoriteIds={favoriteIds} drafts={drafts} emptyTitle={search || filter !== "all" ? "گفتگویی مطابق جستجو و فیلتر نیست." : "گفتگویی وجود ندارد."} onSelect={onSelect} onToggleFavorite={onToggleFavorite} />}
