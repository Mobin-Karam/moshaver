...
export function ConversationListItem({
  ...
  return (
    ...
    >
      ...
      >
        <span
          className={`relative grid size-10 shrink-0 place-items-center rounded-xl text-sm font-bold text-white shadow-sm ${item.type === "group" ? "bg-violet-600" : "bg-brand"}`}
        >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center justify-between gap-2">
            <strong className="truncate">
              {item.type === "group" ? item.title : item.student?.name || "دانش‌آموز"}
            </strong>
          ...
          <span className="mt-1 flex min-w-0 items-center gap-2">
            <small
              className={`block min-w-0 flex-1 truncate ${draft.trim() ? "font-semibold text-amber-700 dark:text-amber-300" : "text-slate-500"}`}
            >
              {preview}
            </small>
