...
function CreateGroupForm({ onCancel, onCreated }: { onCancel: () => void; onCreated: (id: string) => void }) {
  ...
  const users = useQuery({ queryKey: ["chat-users", deferredSearch], enabled: deferredSearch.length > 1, queryFn: () => api.get<ChatUser[]>(`/chat/users?limit=15&search=${encodeURIComponent(deferredSearch)}`) });
...
function GroupManager({ conversationId, close, onChanged }: { conversationId: string; close: () => void; onChanged: () => void }) {
  ...
  const members = useQuery({ queryKey: ["chat-group-members", conversationId, deferredMemberSearch], queryFn: () => api.get<GroupMember[]>(`/chat/groups/${conversationId}/members?limit=50${deferredMemberSearch ? `&search=${encodeURIComponent(deferredMemberSearch)}` : ""}`) });
  const candidates = useQuery({ queryKey: ["chat-group-candidates", conversationId, deferredCandidateSearch], enabled: deferredCandidateSearch.length > 1 && !!detail.data && (detail.data.myRole !== "member" || !!detail.data.permissions.members_can_add_members), queryFn: () => api.get<ChatUser[]>(`/chat/groups/${conversationId}/candidates?limit=15&search=${encodeURIComponent(deferredCandidateSearch)}`) });
  ...
  return <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
    <div className="grid content-start gap-4">
      ...
      <section><div className="mb-2 flex items-center justify-between"><strong>اعضا</strong><Badge tone="blue">{group.memberCount}</Badge></div>{group.memberCount > 12 ? <Input className="mb-2" value={memberSearch} onChange={(event) => setMemberSearch(event.target.value)} placeholder="جستجو میان اعضا" aria-label="جستجو میان اعضای گروه" /> : null}<div className="max-h-72 overflow-auto rounded-md border">{members.data?.length ? members.data.map((member) => <MemberRow key={member.id} member={member} myRole={group.myRole} busy={action.isPending} changeRole={(role) => action.mutate({ method: "patch", path: `/chat/groups/${conversationId}/members/${member.id}`, body: { role } })} remove={() => action.mutate({ method: "delete", path: `/chat/groups/${conversationId}/members/${member.id}` })} transfer={() => action.mutate({ method: "post", path: `/chat/groups/${conversationId}/transfer-owner`, body: { userId: member.id } })} confirm={modal.confirm} />) : <p className="p-3 text-center text-sm text-slate-500">عضوی با این جستجو پیدا نشد.</p>}</div>{group.memberCount > 50 && !memberSearch ? <p className="mt-1 text-xs text-amber-700">برای دسترسی به اعضای بیشتر، نام یا نام کاربری را جستجو کنید.</p> : null}</section>
    ...
    <aside className="grid content-start gap-3">
      ...
      {group.myRole === "owner" ? <Button variant="danger" onClick={() => void modal.confirm({ title: "بایگانی گروه؟", description: "گروه از فهرست گفتگوهای فعال همه اعضا خارج می‌شود.", tone: "danger", confirmLabel: "بایگانی" }).then((ok) => { if (ok) return action.mutateAsync({ method: "patch", path: `/chat/groups/${conversationId}`, body: { archived: true } }).then(close); })}><Archive size={16} /> بایگانی گروه</Button> : <Button variant="danger" onClick={() => void modal.confirm({ title: "ترک گروه؟", description: "برای دسترسی دوباره باید یکی از مدیران شما را به گروه اضافه کند.", tone: "danger", confirmLabel: "ترک گروه" }).then((ok) => { if (ok) return action.mutateAsync({ method: "post", path: `/chat/groups/${conversationId}/leave`, body: {} }).then(close); })}><LogOut size={16} /> ترک گروه</Button>}
    </aside>
  </div>;
}
...
function MemberRow({ member, myRole, busy, changeRole, remove, transfer, confirm }: { member: GroupMember; myRole: GroupRole; busy: boolean; changeRole: (role: GroupRole) => void; remove: () => void; transfer: () => void; confirm: ReturnType<typeof useModal>["confirm"] }) {
  const manageable = myRole === "owner" || (myRole === "admin" && member.role === "member");
  return <div className="flex flex-wrap items-center gap-2 border-b p-2 text-sm"><span className="min-w-0 flex-1"><strong className="block truncate">{member.name}</strong><small className="text-slate-500" dir="ltr">@{member.username}</small></span><Badge tone={member.role === "owner" ? "amber" : member.role === "admin" ? "blue" : "neutral"}>{roleLabel(member.role)}</Badge>{myRole === "owner" && member.role !== "owner" ? <><button type="button" disabled={busy} className="text-xs text-brand" onClick={() => changeRole(member.role === "admin" ? "member" : "admin")}>{member.role === "admin" ? "عضو شود" : "مدیر شود"}</button><button type="button" disabled={busy} aria-label={`انتقال مالکیت به ${member.name}`} onClick={() => void confirm({ title: "انتقال مالکیت گروه؟", description: `${member.name} مالک جدید می‌شود و نقش شما به مدیر تغییر می‌کند.`, tone: "danger", confirmLabel: "انتقال مالکیت" }).then((ok) => ok && transfer())}><Crown size={15} /></button></> : null}{manageable && member.role !== "owner" ? <button type="button" disabled={busy} aria-label={`حذف ${member.name} از گروه`} className="text-rose-700" onClick={() => void confirm({ title: "حذف عضو از گروه؟", description: `${member.name} دسترسی خود به این گفتگو را از دست می‌دهد.`, tone: "danger", confirmLabel: "حذف عضو" }).then((ok) => ok && remove())}><Trash2 size={15} /></button> : null}</div>;
}
