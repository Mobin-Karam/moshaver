...
describe("planner parity helpers", () => {
  it("builds Saturday-first week and exact month ranges", () => {
    expect(plannerRange("2026-08-31", "day")).toEqual({ from: "2026-08-31", to: "2026-08-31" });
    expect(plannerRange("2026-08-31", "week")).toEqual({ from: "2026-08-29", to: "2026-09-04" });
    expect(plannerRange("2026-02-12", "month")).toEqual({ from: "2026-02-01", to: "2026-02-28" });
  });
  it("reports overlaps, late tasks, and overloaded days", () => {
    const plan: Plan = { id: "p1", planDate: "2026-08-31", published: false, tasks: [
  ...
  it("supports list ranges and task filters", () => {
    expect(plannerRange("2026-08-31", "list")).toEqual({ from: "2026-08-01", to: "2026-09-30" });
    const plans: Plan[] = [
      { id: "p1", planDate: "2026-08-31", published: false, tasks: [{ id: "t1", type: "study", title: "فیزیک" }] },
      { id: "p2", planDate: "2026-09-01", published: true, tasks: [{ id: "t2", type: "test", title: "ریاضی" }] },
    ];
  ...
  it("filters completed tasks out of the incomplete view", () => {
    const plans: Plan[] = [{ id: "p1", planDate: "2026-08-31", published: true, tasks: [
  ...
  it("uses the configured Persian calendar for month boundaries", () => {
    expect(plannerRange("2026-08-31", "month", "fa-IR", "persian")).toEqual({ from: "2026-08-23", to: "2026-09-22" });
  });
  it("moves tasks optimistically between planner days", () => {
    const plans: Plan[] = [
      { id: "p1", planDate: "2026-08-31", published: false, tasks: [{ id: "t1", type: "study", start: "08:00", end: "09:00" }] },
      { id: "p2", planDate: "2026-09-01", published: false, tasks: [] },
    ];
    const moved = optimisticMove(plans, { taskId: "t1", planId: "p2", start: "10:00", end: "11:00" });
