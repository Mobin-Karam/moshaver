...
export function AdvisorInboxItem({
    ...
    return (
      <article className="rounded-xl border border-rose-100 bg-rose-50/30 p-3 dark:border-rose-950 dark:bg-rose-950/10">
        <header className="flex items-start justify-between gap-2">
          ...
          {createdAt ? (
            <time className="shrink-0 text-[9px] text-slate-400">{formatDateTime(createdAt)}</time>
          ) : null}
        </header>

        {issue.note ? (
          <p className="mt-2 rounded-lg bg-white/80 p-2 text-xs leading-5 text-slate-600 dark:bg-slate-900/70 dark:text-slate-300">
            {issue.note}
          </p>
        ) : null}
          ...
          <div className="mt-3 grid gap-2 border-t border-rose-100 pt-3 dark:border-rose-950">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                variant="soft"
                onClick={() =>
                  void onIssue({ id: issue.id, status: "open", advisorNote: response.trim() }).then(
                    (ok) => {
                      if (ok) setResponse("");
                    },
                  )
              ...
              <Button
                ...
                onClick={() =>
                  void onIssue({
                    id: issue.id,
                    status: "resolved",
                    advisorNote: response.trim() || undefined,
                  })
                }
              >
  ...
  if (row.kind === "recovery") {
    ...
    return (
      <article className="rounded-xl border border-sky-100 bg-sky-50/30 p-3 dark:border-sky-950 dark:bg-sky-950/10">
        ...
        {recovery.note ? (
          <p className="mt-1 rounded-lg bg-white/80 p-2 text-xs leading-5 text-slate-600 dark:bg-slate-900/70 dark:text-slate-300">
            {recovery.note}
          </p>
        ) : null}
  ...
  const Icon =
    row.kind === "missed" ? Clock3 : row.kind === "review" ? BookOpenCheck : GraduationCap;
    ...
    <article className="rounded-xl border border-slate-200 p-3 dark:border-slate-800">
      <header className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          ...
          {subtitle ? (
            <span className="mt-1 block text-[10px] text-slate-500">{subtitle}</span>
          ) : null}
        </div>
        {createdAt ? (
          <time className="shrink-0 text-[9px] text-slate-400">{formatDateTime(createdAt)}</time>
