...
export function NotificationsPage() {
  ...
  const inbox = useQuery({
    ...
    queryFn: () =>
      api.get<Record<string, unknown[]>>(
        `/admin/advisor-inbox?studentId=${students.studentId}`,
      ),
  });
  const notifications = useQuery({
    queryKey: ["notifications"],
    queryFn: () =>
      api.get<{ items: Notification[]; hasMore?: boolean }>(
  ...
  const rows = [
    ...(inbox.data?.missedTasks ?? []).map((x) => ({
      type: "فعالیت انجام‌نشده",
      x,
    })),
    ...(inbox.data?.reviews ?? []).map((x) => ({ type: "مرور", x })),
    ...(inbox.data?.recoveryRequests ?? []).map((x) => ({
      type: "درخواست ریکاوری",
      x,
    })),
    ...(inbox.data?.examRetryRequests ?? []).map((x) => ({
  ...
  return (
    <div className="grid gap-5">
        ...
        {notifications.data?.items.length ? (
          <div className="grid gap-2">
            {notifications.data.items.map((item) => (
              <button
                key={item.id}
                className={`rounded-md border p-3 text-right ${item.isRead ? "bg-white" : "border-teal-200 bg-teal-50"}`}
                onClick={() => !item.isRead && read.mutate(item.id)}
              >
                <span className="flex items-center justify-between">
                  <strong>{item.title}</strong>
                  {item.isRead ? null : <Badge tone="blue">جدید</Badge>}
                </span>
      ...
      <Card>
        <h3 className="mb-3 font-bold">صندوق پیگیری دانش‌آموز</h3>
        {rows.length ? (
          <div className="grid gap-2">
            {rows.map((row, index) => (
