...
export function HeaderNotifications() {
  ...
  return <ViewportPopover open={open} onOpenChange={setOpen} width={360} className="overflow-hidden" trigger={(props) => <button {...props} type="button" className={`relative grid size-9 place-items-center rounded-md border transition ${notifications.unread ? "border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100" : "border-slate-200 bg-white text-slate-600 hover:bg-slate-50"}`} aria-label={`${notifications.unread.toLocaleString("fa-IR")} اعلان خوانده‌نشده`}>
      <Bell size={18} />
