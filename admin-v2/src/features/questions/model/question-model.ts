...
export type QuestionView = Partial<QuestionDraft> & {
  id?: string;
  text?: string;
...
export function questionDraft(item: QuestionView, index = 0): QuestionDraft {
  return {
    ...
    correctOption: item.correctOption || item.correct_option || item.correctAnswer || "a",
...
export function questionError(draft: QuestionDraft) {
  ...
  if (draft.options.length !== 4 || draft.options.some((option) => !option.trim()))
    return "هر چهار گزینه باید تکمیل شوند.";
  if (draft.options.some((option) => option.trim().length > 1000))
    return "هر گزینه حداکثر می‌تواند ۱۰۰۰ نویسه داشته باشد.";
  const unique = new Set(
    draft.options.map((option) => normalizePersianText(option).trim().toLocaleLowerCase("fa")),
  );
  if (unique.size !== 4) return "گزینه‌های تکراری مجاز نیستند.";
  if (!["a", "b", "c", "d"].includes(draft.correctOption)) return "پاسخ صحیح معتبر نیست.";
  if (!Number.isInteger(draft.sortOrder) || draft.sortOrder < 1)
    return "ترتیب سؤال باید عددی بزرگ‌تر از صفر باشد.";
  return "";
}
