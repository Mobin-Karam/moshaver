...
export function useLearningPageState() {
  ...
  const initialFilter = parseLearningFilter(searchParams.get("status"));
  ...
  function updateLocation(nextStudentId: string, nextFilter = filter, nextSearch = search) {
    ...
    if (params.studentId && nextStudentId !== params.studentId) {
      void navigate(
        {
          pathname: `/admin/students/${encodeURIComponent(nextStudentId)}/learning`,
  ...
  function changeSearch(value: string) {
    ...
    updateLocation(studentId, filter, value);
  }

  function changeFilter(value: LearningFilter) {
    setFilter(value);

    updateLocation(studentId, value);
  }
