...
export function ExamsPage() {
  ...
  return (
    <div className="grid gap-5">
      ...
      {retryRequests.data?.length ? <Card><h3 className="mb-3 font-bold">درخواست‌های تلاش مجدد</h3><div className="grid gap-2">{retryRequests.data.map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-3 rounded-md border p-3"><div><strong>{request.examTitle || "آزمون"}</strong><p className="text-xs text-slate-500">{request.reason || "بدون توضیح"}</p></div><div className="flex gap-2"><Button onClick={() => void modal.confirm({ title: "تأیید تلاش مجدد؟", description: "یک فرصت تازه برای انجام آزمون در اختیار دانش‌آموز قرار می‌گیرد.", confirmLabel: "تأیید درخواست" }).then((confirmed) => confirmed && review.mutate({ id: request.id, status: "approved" }))}><Check size={15} />تأیید</Button><Button variant="danger" onClick={() => void modal.confirm({ title: "رد درخواست تلاش مجدد؟", description: request.reason || "درخواست بدون توضیح ثبت شده است.", tone: "danger", confirmLabel: "رد درخواست" }).then((confirmed) => confirmed && review.mutate({ id: request.id, status: "rejected" }))}><X size={15} />رد</Button></div></div>)}</div></Card> : null}
            ...
            <div className="grid gap-3 md:grid-cols-2">
              {exams.data.map((exam) => (
                <article key={exam.id} className="rounded-md border border-slate-200 bg-slate-50 p-3">
                  ...
                  <div className="mt-3 flex gap-2"><Button className="h-8 flex-1 px-2 text-xs" variant="soft" onClick={() => void modal.confirm({ title: exam.published ? "تبدیل آزمون به پیش‌نویس؟" : "انتشار آزمون؟", description: exam.published ? "آزمون از دسترس دانش‌آموز خارج می‌شود." : "آزمون برای دانش‌آموز قابل مشاهده خواهد شد.", confirmLabel: exam.published ? "پیش‌نویس کن" : "انتشار" }).then((confirmed) => confirmed && update.mutate({ id: exam.id, body: { published: !exam.published } }))}>{exam.published ? "پیش‌نویس کردن" : "انتشار"}</Button><Button className="h-8 px-2" variant="danger" onClick={() => void modal.confirm({ title: "حذف آزمون؟", description: `آزمون «${exam.title}» و داده‌های وابسته حذف می‌شوند.`, tone: "danger", confirmLabel: "حذف" }).then((confirmed) => confirmed && remove.mutate(exam.id))}><Trash2 size={14} /></Button></div>
                  <div className="mt-3 border-t pt-3"><div className="mb-2 flex items-center justify-between"><strong className="text-xs">بودجه‌بندی</strong><button className="text-xs text-brand" onClick={() => modal.open({ title: "افزودن بودجه‌بندی", description: exam.title, content: <SyllabusForm onCancel={modal.close} onSubmit={(subject, description) => { addSyllabus.mutate({ examId: exam.id, subject, description }); modal.close(); }} /> })}>+ افزودن</button></div>{exam.syllabus?.map((item) => <div key={item.id} className="mb-1 flex justify-between rounded bg-white p-2 text-xs"><span>{item.subject}: {item.description}</span><button className="text-rose-700" onClick={() => void modal.confirm({ title: "حذف بودجه‌بندی؟", description: `${item.subject}: ${item.description}`, tone: "danger", confirmLabel: "حذف" }).then((confirmed) => confirmed && deleteSyllabus.mutate(item.id))}>حذف</button></div>)}</div>
                </article>
              ))}
            </div>
