...
export function ExamList({
  ...
  const allSelected = !!filtered.length && filtered.every((exam) => selected.includes(exam.id));
    ...
    <Card>
      <div className="mb-3 flex items-center justify-between">
        {onSelectAll ? (
          ...
          <span className="text-sm font-bold">فهرست آزمون‌ها</span>
        )}
      ...
      {loading ? (
        <div className="grid gap-3 lg:grid-cols-2" aria-label="در حال دریافت آزمون‌ها">
          {[1, 2, 3, 4].map((item) => (
