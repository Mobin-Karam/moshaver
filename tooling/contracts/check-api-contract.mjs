...
const migrationRoles = new Set([...migration.matchAll(/\["([A-Z_]+)",\s*"[^"]+",\s*[01]\]/g)].map((item) => item[1]));

for (const role of migrationRoles) {
  if (!contractRoles.has(role)) errors.push(`API contract is missing backend role ${role}`);
}
for (const role of contractRoles) {
  if (!migrationRoles.has(role)) errors.push(`API contract role ${role} is not seeded by the backend identity migration`);
}

const capabilityPrefixes = new Set([
  "organization", "users", "students", "student", "subjects", "studentSubjects", "learning", "plans", "tasks", "exams", "questions", "syllabus", "retry_requests", "quizzes", "quiz_questions", "mistakes", "chat", "reports", "analytics", "recommendations", "import", "export", "release", "audit", "system", "database",
]);
const capabilityActions = new Set(["read", "manage", "create", "update", "archive", "publish", "delete", "assign", "send", "moderate", "preview", "commit", "backup", "restore", "review"]);
const ignoredEventLiterals = new Set(["system.update", "plan.updated", "exam.created", "notification.created", "chat.message.created"]);
...
for (const capability of backendCapabilities) {
  if (!contractCapabilities.has(capability)) errors.push(`API contract is missing backend capability ${capability}`);
}
...
if (!baseMatch || !versionMatch) errors.push("API contract must export API_BASE_PATH and API_VERSION.");
  ...
  if (baseMatch[1] !== `/${prefixMatch?.[1] ?? ""}`) errors.push(`API_BASE_PATH ${baseMatch[1]} does not match backend prefix /${prefixMatch?.[1] ?? "<missing>"}`);
...
console.log(`API contract OK: ${contractRoles.size} roles, ${contractCapabilities.size} declared capabilities, ${backendCapabilities.size} capabilities observed in backend source, ${baseMatch?.[1]}.`);
