(function (global) {
  ...
  function createMoshaverApiClient(options) {
    options = options || {};
    var base =
      (global.APP_CONFIG && global.APP_CONFIG.API_BASE_URL) || "/api/v1";
    var csrfKey = options.csrfKey || "moshaver_csrf";
    ...
    function setCsrf(v) {
      try {
        if (v) sessionStorage.setItem(csrfKey, v);
        else sessionStorage.removeItem(csrfKey);
      } catch (e) {}
    }
    function isMutating(method) {
      return ["POST", "PUT", "PATCH", "DELETE"].indexOf(
    ...
    function refreshCsrf(done) {
      ...
      x.onerror = function () {
        var err = {
            status: 0,
            message: "اینترنت یا سرور در دسترس نیست.",
            code: "NETWORK",
          },
          list = refreshWaiters.slice();
        refreshWaiters = [];
        refreshInFlight = false;
    ...
    function request(method, path, body, callback, requestOptions) {
      ...
      function send() {
        ...
        xhr.onreadystatechange = function () {
          ...
          if (xhr.status >= 200 && xhr.status < 300 && payload && payload.ok) {
            ...
            finish(null, payload.data, xhr.status);
            return;
          }
          ...
          if (
            ...
            isMutating(method) &&
            !csrfRetried &&
            !requestOptions.noCsrfRetry
          ) {
            csrfRetried = true;
            refreshCsrf(function (refreshErr) {
              if (ctrl.aborted) return;
              if (refreshErr) {
                finish(refreshErr, null, refreshErr.status);
                return;
              }
              send();
            });
          ...
          if (xhr.status === 401) {
            setCsrf("");
            if (!requestOptions.suppressAuthFailure) notifyAuthFailure(err, path);
          }
    ...
    function setBase(nextBase) {
      var value=String(nextBase||"").trim().replace(/\/$/,"");
      if(!value) return base;
      abortAll(); setCsrf(""); base=value; return base;
    }
  ...
  global.MoshaverApiClient = {
    create: createMoshaverApiClient,
  };
})(window);
