...
function mapPlan(plan, studentId) {
  ...
  var tasks = db
    .prepare(
      `SELECT t.*,tc.status AS completion_status,tc.actual_minutes,tc.actual_tests,tc.note AS completion_note
    FROM tasks t LEFT JOIN task_completions tc ON tc.task_id=t.id AND tc.student_id=?
    WHERE t.plan_id=? ORDER BY t.sort_order,t.start_time`,
    )
...
function examDefaultOpen(e) {
  return e.open_at || e.iso_date + "T00:00:00+03:30";
}
function examDefaultClose(e) {
  return e.close_at || e.iso_date + "T23:59:59+03:30";
}
...
function examAccess(e, studentId) {
  ...
  lastApproved =
    db
      ...
      .get(e.id, studentId) || null;
  var baseMax = Math.max(1, Number(e.max_attempts || 1)),
    allowed = baseMax + approved,
    openAt = examDefaultOpen(e),
    closeAt = examDefaultClose(e),
    nowMs = Date.now(),
    inWindow =
      nowMs >= new Date(openAt).getTime() &&
      nowMs <= new Date(closeAt).getTime();
  var retryWindow = false;
  if (
    attempts >= baseMax &&
    attempts < allowed &&
    lastApproved &&
    lastApproved.resolvedAt
  ) {
    retryWindow =
      nowMs <= new Date(lastApproved.resolvedAt).getTime() + 24 * 3600000;
  }
  var published = Number(e.published == null ? 1 : e.published) === 1,
    questionCount = qz ? Number(qz.question_count || 0) : 0,
  ...
  else if (!qz || !questionCount) reason = "no_questions";
  else if (activeRun) reason = "resume";
  else if (attempts >= allowed)
    reason = pending ? "retry_pending" : "attempt_limit";
  else if (!inWindow && !retryWindow)
    reason = nowMs < new Date(openAt).getTime() ? "not_open" : "closed";
  return {
    quiz: qz,
...
function mapExam(e, studentId, adminMode) {
  ...
  var item = {
    ...
    openAt: examDefaultOpen(e),
    closeAt: examDefaultClose(e),
    durationMinutes: Math.max(1, Number(e.duration_minutes || 120)),
    maxAttempts: Math.max(1, Number(e.max_attempts || 1)),
    instructions: e.instructions || "",
    published: Number(e.published == null ? 1 : e.published) === 1,
    syllabus: db
      .prepare(
  ...
  if (adminMode && access) {
    item.delivery.lastApproved = access.lastApproved;
  }
  return item;
}
function getExams(studentId, adminMode) {
  ...
  if (studentId)
    exams = db
      .prepare(
        "SELECT * FROM exams WHERE student_id=? OR student_id IS NULL ORDER BY iso_date,created_at",
      )
      .all(studentId);
  else
    exams = db
      .prepare("SELECT * FROM exams ORDER BY iso_date,created_at")
      .all();
  return exams.map(function (e) {
    return mapExam(e, studentId, !!adminMode);
  });
}
function quizPayload(qz) {
  var out = {
    ...
    title: qz.title,
    subject: qz.subject || "",
    durationMinutes: Number(qz.duration_minutes || qz.durationMinutes || 20),
    examId: qz.exam_id || qz.examId || null,
  };
  out.questions = db
    .prepare(
...
function createOrResumeQuizRun(qz, studentId, deviceLabel) {
  ...
  ).run(runId, qz.id, studentId, t, t);
  touchPresence(studentId, "quiz", null, null, str(deviceLabel, 120));
  recordActivity(studentId, "quiz.started", "quiz", qz.id, { runId: runId });
...
function getExamProgress(studentId, examId) {
  var exam = db
    .prepare(
      "SELECT * FROM exams WHERE id=? AND (student_id=? OR student_id IS NULL)",
    )
...
function getPlanMetrics(plan) {
  ...
  plan.tasks.forEach(function (t) {
    var a = String(t.start || "00:00").split(":"),
      b = String(t.end || "00:00").split(":");
    var m =
      Number(b[0]) * 60 + Number(b[1]) - (Number(a[0]) * 60 + Number(a[1]));
    if (m > 0) plannedM += m;
    plannedT += Number(t.testCount || 0);
    if (t.completion) {
      if (t.completion.status === "done") doneN++;
      else if (t.completion.status === "partial") partial++;
      actualM += Number(t.completion.actualMinutes || 0);
      actualT += Number(t.completion.actualTests || 0);
    }
  });
  return {
    totalTasks: total,
...
function getMistakes(studentId, limit) {
  ...
  return db
    .prepare(
      `SELECT qa.id,qa.attempt_id AS attemptId,qa.question_id AS questionId,qa.selected_option AS selectedOption,
    ...
    JOIN quizzes q ON q.id=att.quiz_id WHERE att.student_id=? AND qa.is_correct=0 ORDER BY att.submitted_at DESC LIMIT ?`,
    )
    .all(studentId, n);
}
...
function studentDashboard(studentId, date, includeDraft) {
  ...
  var recent = db
    .prepare(
      "SELECT * FROM quiz_attempts WHERE student_id=? ORDER BY submitted_at DESC LIMIT 6",
    )
    .all(studentId);
  return {
    student: {
    ...
    pendingRecovery: db
      .prepare(
        "SELECT COUNT(*) AS n FROM recovery_requests WHERE student_id=? AND status='pending'",
      )
      .get(studentId).n,
    recentAttempts: recent.map(function (r) {
      return {
        id: r.id,
        quizId: r.quiz_id,
        correct: r.correct,
...
function notifyStudent(studentId, title, body) {
  ...
  try {
    ...
    ).run(id, studentId, item.title, item.body, t);
...
function touchPresence(studentId, state, taskId, sessionId, deviceLabel) {
  var previous = db
      ...
      .get(studentId),
    t = now(),
    nextState = str(state, 30) || "online";
  db.prepare(
    `INSERT INTO student_presence (student_id,state,active_task_id,active_session_id,last_seen_at,device_label) VALUES (?,?,?,?,?,?)
...
function getPresence(studentId) {
  ...
  return {
    ...
    activeTaskId: p.active_task_id || null,
...
function activeStudySession(studentId) {
  var r = db
    .prepare(
      `SELECT ss.*,t.subject,t.title,t.start_time,t.end_time FROM study_sessions ss LEFT JOIN tasks t ON t.id=ss.task_id
    WHERE ss.student_id=? AND ss.status='active' ORDER BY ss.started_at DESC LIMIT 1`,
    )
    .get(studentId);
  if (!r) return null;
  return {
    ...
    subject: r.subject || "",
    title: r.title || "",
    plannedStart: r.start_time || "",
    plannedEnd: r.end_time || "",
    note: r.note || "",
  };
}
...
function getReadAt(conversationId, userId) {
  var r = db
    .prepare(
      "SELECT last_read_at FROM chat_reads WHERE conversation_id=? AND user_id=?",
    )
...
function adminChatList(adminUser) {
  var students = db
    .prepare("SELECT id,name,grade FROM students WHERE active=1 ORDER BY name")
    .all();
  return students.map(function (st) {
    var c = getOrCreateConversation(st.id),
      last = db
        .prepare(
    ...
    return {
      id: c.id,
      student: { id: st.id, name: st.name, grade: st.grade },
      lastMessage: last ? mapChatMessage(last, null, adminUser.id) : null,
      unread: conversationUnread(c.id, adminUser),
      presence: getPresence(st.id),
    };
  });
}
...
function normalizeImportPayload(input, fallbackStudentId) {
  ...
  if (
    studentId &&
    !db.prepare("SELECT id FROM students WHERE id=?").get(studentId)
  )
    errors.push("Student not found: " + studentId);
  var plans = [],
    planSeen = {};
  (Array.isArray(src.plans) ? src.plans : []).forEach(function (p, pi) {
    var date = str(p.planDate, 10);
    if (!isoDateValid(date)) {
      errors.push("plans[" + pi + "].planDate must be YYYY-MM-DD.");
      return;
    }
    if (planSeen[date]) warnings.push("Duplicate imported plan date: " + date);
    planSeen[date] = 1;
    var tasks = [];
    (Array.isArray(p.tasks) ? p.tasks : []).forEach(function (t, ti) {
      var st = str(t.start, 5),
        en = str(t.end, 5);
      if (!timeValid(st) || !timeValid(en)) {
        errors.push("Invalid time in " + date + " task " + (ti + 1));
        return;
      }
      if (en <= st)
        warnings.push(
          "Task end time is not after start time: " +
            date +
            " " +
            st +
            "-" +
      ...
      tasks.push({
        ...
        type: str(t.type, 30) || "study",
        subject: str(t.subject, 150),
        title: str(t.title, 300),
        pages: str(t.pages, 120),
        testCount: Math.max(0, num(t.testCount, 0)),
        note: str(t.note, 1500),
        quizId: str(t.quizId, 120) || null,
        examId: str(t.examId, 120) || null,
        examRef: str(t.examRef, 120) || null,
        sortOrder: num(t.sortOrder, ti + 1),
      });
    });
    for (var a = 0; a < tasks.length; a++)
      for (var b = a + 1; b < tasks.length; b++)
        if (tasks[a].start < tasks[b].end && tasks[b].start < tasks[a].end)
          conflicts.push(
            date +
              ": " +
              tasks[a].start +
    ...
    plans.push({
      planDate: date,
      jalaliId: str(p.jalaliId, 40),
      dayLabel: str(p.dayLabel, 60),
      persianDate: str(p.persianDate, 100),
      title: str(p.title, 250),
      motivationText: str(p.motivationText || p.motivation, 600),
  ...
  (Array.isArray(src.exams) ? src.exams : []).forEach(function (e, ei) {
    var date = str(e.isoDate, 10),
      title = str(e.title, 250);
    if (!title || !isoDateValid(date)) {
      errors.push("exams[" + ei + "] needs title and isoDate.");
      return;
    }
    var openAt = str(e.openAt, 80) || date + "T08:00:00+03:30",
      closeAt = str(e.closeAt, 80) || date + "T13:00:00+03:30";
    if (
      !dateTimeValid(openAt) ||
      !dateTimeValid(closeAt) ||
      new Date(closeAt) <= new Date(openAt)
    ) {
      errors.push("exams[" + ei + "] has invalid openAt/closeAt.");
      return;
    }
    var syllabus = [];
    (Array.isArray(e.syllabus) ? e.syllabus : []).forEach(function (x) {
      if (str(x.subject, 150) && str(x.description, 1200))
        syllabus.push({
          subject: str(x.subject, 150),
          description: str(x.description, 1200),
          required: x.required !== false,
          track: str(x.track, 120),
        });
    });
    var rawQuestions = Array.isArray(e.questions)
        ? e.questions
        : e.quiz && Array.isArray(e.quiz.questions)
          ? e.quiz.questions
          : [],
      questions = [];
    rawQuestions.forEach(function (x, qi) {
      var opts = Array.isArray(x.options) ? x.options : [];
      var co = str(x.correctOption || x.answer, 20).toLowerCase();
      if (/^[0-3]$/.test(co)) co = ["a", "b", "c", "d"][Number(co)];
      if (
        !str(x.question || x.q, 2000) ||
        opts.length !== 4 ||
        ["a", "b", "c", "d"].indexOf(co) < 0
      ) {
        errors.push(
          "exams[" +
            ei +
      ...
      questions.push({
        question: str(x.question || x.q, 2000),
        options: [
          str(opts[0], 1000),
          str(opts[1], 1000),
          str(opts[2], 1000),
        ...
        correctOption: co,
        explanation: str(x.explanation, 2000),
        sortOrder: num(x.sortOrder, qi + 1),
      });
    });
    ...
    if (
      ...
      db
        .prepare(
          "SELECT id FROM exams WHERE student_id=? AND title=? AND iso_date=? LIMIT 1",
        )
        .get(studentId, title, date)
    )
      warnings.push("Existing exam found: " + title + " " + date + ".");
    exams.push({
      ref: str(e.ref, 120) || null,
      title: title,
      persianDate: str(e.persianDate, 100) || date,
      isoDate: date,
      note: str(e.note, 1500),
      status: str(e.status, 30) || "upcoming",
      published: e.published !== false,
      openAt: openAt,
      closeAt: closeAt,
      durationMinutes: Math.max(1, num(e.durationMinutes, 120)),
      maxAttempts: 1,
      instructions: str(e.instructions, 3000),
      syllabus: syllabus,
      questions: questions,
    });
  });
  ...
  exams.forEach(function (e) {
    if (e.ref) {
      if (knownExamRefs[e.ref]) errors.push("Duplicate exam ref: " + e.ref);
      knownExamRefs[e.ref] = 1;
    }
  });
  plans.forEach(function (p) {
    p.tasks.forEach(function (t) {
      if (t.examRef && !knownExamRefs[t.examRef])
        errors.push(
          "Unknown examRef " + t.examRef + " in plan " + p.planDate + ".",
        );
    });
  });
  var taskCount = 0;
  ...
  try {
    ...
    normalized.exams.forEach(function (e) {
      var existing = db
        ...
        .get(studentId, e.title, e.isoDate);
      if (existing) {
        if (!opts.replaceExistingExams)
          throw new Error("EXAM_EXISTS:" + e.title);
        db.prepare("DELETE FROM exams WHERE id=?").run(existing.id);
      }
      var eid = security.id("exam");
      db.prepare(
        ...
        e.maxAttempts,
        e.instructions,
        opts.publishImported || e.published ? 1 : 0,
      );
      ...
      var qid = security.id("quiz");
      db.prepare(
        "INSERT INTO quizzes (id,title,subject,duration_minutes,exam_id,active,created_at,updated_at) VALUES (?,?,?,?,?,1,?,?)",
      ).run(qid, e.title, "آزمون اصلی", e.durationMinutes, eid, t, t);
      e.questions.forEach(function (x) {
        db.prepare(
    ...
    normalized.plans.forEach(function (p) {
      var existing = db
        .prepare("SELECT id FROM plans WHERE student_id=? AND plan_date=?")
        .get(studentId, p.planDate);
      if (existing) {
        if (!opts.replaceExistingPlans)
          throw new Error("PLAN_EXISTS:" + p.planDate);
        db.prepare("DELETE FROM plans WHERE id=?").run(existing.id);
      }
      var pid = security.id("plan");
      db.prepare(
        ...
        p.title,
        p.motivationText || "",
        opts.publishImported ? 1 : p.published ? 1 : 0,
        t,
        t,
      );
      p.tasks.forEach(function (x, idx) {
        var linkedExamId = x.examId || null;
        if (!linkedExamId && x.examRef)
          linkedExamId = examRefMap[x.examRef] || null;
        if (!linkedExamId && x.type === "exam")
          linkedExamId = examRefMap[p.planDate + "|" + x.title] || null;
        if (
          linkedExamId &&
          !db
            .prepare(
              "SELECT id FROM exams WHERE id=? AND (student_id=? OR student_id IS NULL)",
            )
    ...
    if (opts.publishImported) {
      notifyStudent(
        studentId,
        "برنامه و آزمون جدید",
        "اطلاعات جدید توسط مشاور منتشر شد.",
      );
    ...
    audit(user, "import", "student_data", iid, {
      studentId: studentId,
      plans: createdPlans,
      tasks: createdTasks,
      exams: createdExams,
...
router.add(
  ...
  async function (req, res, match, body) {
    var username = str(body.username, 100),
      ...
      keys = loginKeys(req, username),
      blocked = isLoginBlocked(keys);
    if (!username || !password)
      return fail(res, 400, "VALIDATION", "نام کاربری و رمز عبور لازم است.");
    if (blocked) {
      ...
      return fail(
        ...
        429,
        "RATE_LIMITED",
        "تلاش‌های ورود بیش از حد است. چند دقیقه بعد دوباره تلاش کنید.",
        { blockedUntil: blocked },
      );
    }
    var u = db
    ...
    if (verified.needsRehash) {
      try {
        ...
        ).run(await security.hashPassword(password), now(), u.id);
      } catch (e) {
...
router.add(
  ...
  async function (req, res, match, body, user) {
    var current = str(body.currentPassword, 300),
      next = str(body.newPassword, 300);
    if (next.length < 12)
      return fail(
        res,
        400,
        "WEAK_PASSWORD",
        "رمز جدید باید حداقل ۱۲ نویسه باشد.",
      );
    ...
    if (!verified.ok)
      return fail(res, 401, "INVALID_CREDENTIALS", "رمز فعلی درست نیست.");
    ...
    ok(res, { changed: true, otherSessionsRevoked: true });
  },
);
...
router.add(
  ...
  ["admin", "student"],
  function (req, res, match, body, user) {
    if (match[1] === user.session_id)
      return fail(
        res,
        400,
        "CURRENT_SESSION",
        "برای خروج از نشست فعلی از دکمه خروج استفاده کنید.",
      );
    var r = db
      .prepare("DELETE FROM sessions WHERE id=? AND user_id=?")
      .run(match[1], user.id);
    audit(user, "SESSION_REVOKED", "session", match[1], {});
    ok(res, { revoked: !!r.changes });
  },
);
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    ok(
      res,
      rows.map(function (p) {
        return mapPlan(p, user.student_id);
      }),
    );
  },
);
...
router.add(
  ...
  function (req, res, match, body, user) {
    if (!bucketAllow("presence:" + user.id, 4, 60000))
      return fail(res, 429, "RATE_LIMITED", "به‌روزرسانی وضعیت بیش از حد است.");
    var state =
...
router.add(
  ...
  function (req, res, match, body, user) {
    if (!bucketAllow("activity:" + user.id, 40, 60000))
      return fail(res, 429, "RATE_LIMITED", "رویدادهای زیادی ارسال شده است.");
    var allowed = {
      "task.opened": 1,
      "task.done": 1,
      "task.partial": 1,
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    var end = now(),
      minutes = Math.max(
        0,
        Math.round((new Date(end) - new Date(ss.started_at)) / 60000),
      );
    db.prepare(
      "UPDATE study_sessions SET ended_at=?,status='finished',actual_minutes=?,last_heartbeat_at=?,note=?,updated_at=? WHERE id=?",
    ).run(end, minutes, end, str(body.note, 500) || ss.note, end, ss.id);
    touchPresence(
      user.student_id,
      "online",
      null,
    ...
    ok(res, { id: ss.id, actualMinutes: minutes, endedAt: end });
  },
);
router.add(
  "POST",
...
router.add(
  ...
  ["student"],
  function (req, res, match, body, user) {
    ok(res, getExams(user.student_id, false));
  },
);
...
router.add(
  ...
  ["student"],
  function (req, res, match, body, user) {
    var data = getExamProgress(user.student_id, match[1]);
    if (!data) return fail(res, 404, "NOT_FOUND", "آزمون پیدا نشد.");
    ok(res, data);
  },
);
  ...
  function (req, res, match, body, user) {
    ...
    if (!access.quiz || !access.questionCount)
      return fail(
        ...
        409,
        "EXAM_NOT_READY",
        "سؤال‌های آزمون هنوز توسط مشاور آماده نشده است.",
      );
    if (!access.canStart) {
      var messages = {
        ...
        attempt_limit:
          "این آزمون فقط یک‌بار قابل انجام است. برای تلاش مجدد درخواست بفرستید.",
      ...
      return fail(
        ...
        409,
        "EXAM_UNAVAILABLE",
        messages[access.reason] || "آزمون در حال حاضر قابل اجرا نیست.",
        {
          reason: access.reason,
          openAt: access.openAt,
          closeAt: access.closeAt,
  ...
  function (req, res, match, body, user) {
    ...
    if (access.attemptsUsed < access.allowedAttempts)
      return fail(
        ...
        409,
        "RETRY_NOT_NEEDED",
        "هنوز یک تلاش مجاز برای این آزمون دارید.",
      );
    if (access.retryRequest)
      return fail(
        ...
        409,
        "RETRY_PENDING",
        "یک درخواست تلاش مجدد از قبل در انتظار بررسی است.",
      );
    var id = security.id("examretry"),
      t = now(),
    ...
    ).run(id, e.id, user.student_id, message, t, t);
    recordActivity(user.student_id, "exam.retry_requested", "exam", e.id, {
      requestId: id,
    });
...
router.add(
  ...
  ["student"],
  function (req, res, match, body, user) {
    if (!db.prepare("SELECT id FROM exam_syllabus WHERE id=?").get(match[1]))
      return fail(res, 404, "NOT_FOUND", "بودجه آزمون پیدا نشد.");
    var status =
      ["unread", "read", "tested", "review", "mastered"].indexOf(body.status) >=
      0
        ? body.status
        : "read";
    var t = now();
    db.prepare(
      `INSERT INTO syllabus_progress (student_id,syllabus_id,status,accuracy,note,updated_at) VALUES (?,?,?,?,?,?)
    ON CONFLICT(student_id,syllabus_id) DO UPDATE SET status=excluded.status,accuracy=excluded.accuracy,note=excluded.note,updated_at=excluded.updated_at`,
      ...
      match[1],
      status,
      Math.min(100, Math.max(0, num(body.accuracy, 0))),
      str(body.note, 1000),
      t,
    );
    if (status === "read" || status === "tested")
      scheduleReviews(user.student_id, match[1], todayIso());
    recordActivity(
      user.student_id,
      "syllabus." + status,
      "syllabus",
      match[1],
      { accuracy: Math.min(100, Math.max(0, num(body.accuracy, 0))) },
    );
    ok(res, {
      syllabusId: match[1],
      status: status,
...
router.add(
  ...
  function (req, res, match, body, user) {
    var q = query(req);
    ok(res, getMistakes(user.student_id, q.limit));
  },
);
router.add(
  "PATCH",
  ...
  function (req, res, match, body, user) {
    var a = db
      ...
      .get(match[1], user.student_id);
    if (!a) return fail(res, 404, "NOT_FOUND", "اشتباه پیدا نشد.");
    db.prepare("UPDATE quiz_answers SET error_reason=? WHERE id=?").run(
      str(body.errorReason, 100),
      a.id,
    );
    ok(res, { id: a.id, errorReason: str(body.errorReason, 100) });
  },
);
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    ok(
      res,
      Object.assign({}, getReport(user.student_id, date), { serverMetrics: m }),
      201,
    );
  },
);
  ...
  function (req, res, match, body, user) {
    ...
    if (qz.exam_id) {
      var e = db
        ...
        .get(qz.exam_id, user.student_id);
      if (!e) return fail(res, 404, "NOT_FOUND", "آزمون اصلی پیدا نشد.");
      var access = examAccess(e, user.student_id);
      if (!access.canStart)
        return fail(
          res,
          409,
          "EXAM_UNAVAILABLE",
          "این آزمون در حال حاضر قابل اجرا نیست.",
          { reason: access.reason },
        );
    }
    var run = createOrResumeQuizRun(qz, user.student_id, body.deviceLabel);
    ok(res, run, run.resumed ? 200 : 201);
  },
);
...
router.add(
  ...
  function (req, res, match) {
    var qz = db
      .prepare(
        "SELECT id,title,subject,duration_minutes AS durationMinutes,exam_id AS examId FROM quizzes WHERE id=? AND active=1",
      )
      .get(match[1]);
    if (!qz) return fail(res, 404, "NOT_FOUND", "آزمون پیدا نشد.");
    qz.questions = db
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    var runId = str(body.runId, 120),
      run = db
        .prepare(
          "SELECT * FROM quiz_runs WHERE id=? AND quiz_id=? AND student_id=? AND status='active'",
        )
        .get(runId, qz.id, user.student_id);
    if (!run)
      return fail(
        res,
        409,
        "QUIZ_RUN",
        "جلسه آزمون معتبر نیست؛ آزمون را دوباره شروع کنید.",
      );
    var correct = 0,
      wrong = 0,
      blank = 0;
    ...
    var t = now(),
      duration = Math.max(
        0,
        Math.round((new Date(t) - new Date(run.started_at)) / 1000),
      );
    db.exec("BEGIN");
    try {
      db.prepare(
      ...
      questions.forEach(function (q) {
        var a = map[q.id] || {};
        var selected =
          ["a", "b", "c", "d"].indexOf(a.selectedOption) >= 0
            ? a.selectedOption
            : null;
        ...
        insert.run(
          ...
          selected,
          isCorrect,
          str(a.errorReason, 100),
        );
      });
      var total = questions.length;
      var percent = total ? Math.round((correct * 100) / total) : 0;
      db.prepare(
        "UPDATE quiz_attempts SET correct=?,wrong=?,blank=?,percent=? WHERE id=?",
      ).run(correct, wrong, blank, percent, attemptId);
      db.prepare(
        "UPDATE quiz_runs SET status='submitted',submitted_at=? WHERE id=?",
      ).run(t, run.id);
      if (qz.exam_id) {
        var linkedTasks = db
          .prepare(
            `SELECT t.id FROM tasks t JOIN plans p ON p.id=t.plan_id WHERE t.exam_id=? AND p.student_id=?`,
          )
          .all(qz.exam_id, user.student_id);
        linkedTasks.forEach(function (lt) {
          db.prepare(
      ...
      var review = db
        .prepare(
          `SELECT qa.id AS answerId,qa.question_id AS questionId,qq.question_text AS question,qa.selected_option AS selectedOption,qq.correct_option AS correctOption,qq.explanation,qa.is_correct AS isCorrect,qa.error_reason AS errorReason
      FROM quiz_answers qa JOIN quiz_questions qq ON qq.id=qa.question_id WHERE qa.attempt_id=? ORDER BY qq.sort_order`,
        )
        .all(attemptId);
      touchPresence(user.student_id, "online", null, null, "");
      recordActivity(user.student_id, "quiz.completed", "quiz", qz.id, {
        attemptId: attemptId,
        percent: percent,
        correct: correct,
        wrong: wrong,
...
router.add(
  ...
  function (req, res, match, body, user) {
    ok(
      ...
      db
        .prepare(
          `SELECT qa.id,qa.quiz_id AS quizId,q.title,qa.correct,qa.wrong,qa.blank,qa.percent,qa.submitted_at AS submittedAt
    FROM quiz_attempts qa JOIN quizzes q ON q.id=qa.quiz_id WHERE qa.student_id=? ORDER BY qa.submitted_at DESC LIMIT 50`,
        )
        .all(user.student_id),
    );
  },
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    var qy = query(req),
      rows = chatMessages(c.id, qy.limit),
      other =
        user.role === "admin"
          ? db
              .prepare(
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    var text = str(body.text, 3000),
      reply = str(body.replyToId, 120) || null;
    if (!text) return fail(res, 400, "VALIDATION", "متن پیام لازم است.");
    if (
      reply &&
      !db
        .prepare("SELECT 1 FROM chat_messages WHERE id=? AND conversation_id=?")
        .get(reply, c.id)
    )
    ...
    var id = security.id("msg"),
      t = now();
    db.prepare(
      "INSERT INTO chat_messages (id,conversation_id,sender_user_id,sender_role,message_text,reply_to_id,created_at,edited_at,deleted_at) VALUES (?,?,?,?,?,?,?,NULL,NULL)",
...
router.add(
  ...
  function (req, res) {
    ok(
      ...
      db
        .prepare(
          `SELECT s.*,u.username FROM students s LEFT JOIN users u ON u.student_id=s.id AND u.role='student' ORDER BY s.created_at`,
        )
        .all(),
    );
  },
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    try {
      db.prepare(
        "INSERT INTO students (id,name,grade,major,target_major,target_city,rank_goal,daily_capacity,active,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,1,?,?)",
      ).run(
        sid,
        name,
      ...
      db.prepare(
        "INSERT INTO users (id,username,password_hash,role,display_name,student_id,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,1,?,?)",
      ).run(
        uid,
        username,
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    var existing = db
        .prepare("SELECT * FROM plans WHERE student_id=? AND plan_date=?")
        .get(sid, date),
      pid = existing ? existing.id : security.id("plan"),
      t = now();
    if (existing) {
      db.prepare(
    ...
    if (existing)
      emitStudent(sid, "plan.updated", { planId: pid, planDate: date });
    else if (boolInt(body.published))
      emitStudent(sid, "plan.published", { planId: pid, planDate: date });
    audit(user, existing ? "update" : "create", "plan", pid, body);
    ok(
      res,
      mapPlan(db.prepare("SELECT * FROM plans WHERE id=?").get(pid), sid),
      existing ? 200 : 201,
    );
  },
);
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var p = db.prepare("SELECT * FROM plans WHERE id=?").get(match[1]);
    if (!p) return fail(res, 404, "NOT_FOUND", "برنامه پیدا نشد.");
    var map = {
      title: "title",
      dayLabel: "day_label",
      ...
      if (Object.prototype.hasOwnProperty.call(body, k)) {
        var col = map[k],
          val =
            col === "published"
              ? boolInt(body[k])
              : str(body[k], k === "motivationText" ? 600 : 300);
        db.prepare(
          "UPDATE plans SET " + col + "=?,updated_at=? WHERE id=?",
        ).run(val, now(), p.id);
      }
    });
    if (
      ...
      notifyStudent(
        ...
        "برنامه " + p.plan_date + " توسط مشاور منتشر شد.",
      );
      emitStudent(p.student_id, "plan.published", {
        planId: p.id,
        planDate: p.plan_date,
      });
    } else {
      emitStudent(p.student_id, "plan.updated", {
        planId: p.id,
        planDate: p.plan_date,
      });
    }
    audit(user, "update", "plan", p.id, body);
    ok(
      res,
      mapPlan(
        db.prepare("SELECT * FROM plans WHERE id=?").get(p.id),
        p.student_id,
      ),
    );
  },
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var source = db.prepare("SELECT * FROM plans WHERE id=?").get(match[1]);
    if (!source) return fail(res, 404, "NOT_FOUND", "برنامه مبدا پیدا نشد.");
    var date = str(body.planDate, 10);
    if (!isoDateValid(date))
    ...
    if (
      db
        .prepare("SELECT id FROM plans WHERE student_id=? AND plan_date=?")
        .get(source.student_id, date)
    )
      return fail(
        res,
        409,
        "DUPLICATE",
        "برای تاریخ مقصد از قبل برنامه وجود دارد.",
      );
    var pid = security.id("plan"),
      t = now();
    db.exec("BEGIN");
    try {
      ...
      var tasks = db
          .prepare(
            "SELECT * FROM tasks WHERE plan_id=? ORDER BY sort_order,start_time",
          )
          .all(source.id),
        ins = db.prepare(
          "INSERT INTO tasks (id,plan_id,start_time,end_time,type,subject,title,pages,test_count,note,quiz_id,exam_id,sort_order,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ...
    audit(user, "duplicate", "plan", pid, { sourceId: source.id, date: date });
    ok(
      res,
      mapPlan(
        db.prepare("SELECT * FROM plans WHERE id=?").get(pid),
        source.student_id,
      ),
      201,
    );
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    var id = security.id("task"),
      t = now(),
      order = num(body.sortOrder, 999);
    ...
    if (
      ...
      !db
        ...
        .get(linkedExamId, p.student_id)
    )
      return fail(res, 400, "VALIDATION", "آزمون مرتبط معتبر نیست.");
    db.prepare(
      "INSERT INTO tasks (id,plan_id,start_time,end_time,type,subject,title,pages,test_count,note,quiz_id,exam_id,sort_order,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ).run(
      id,
    ...
    audit(user, "create", "task", id, body);
    ok(res, mapPlan(p, p.student_id), 201);
  },
);
...
router.add(
  ...
  function (req, res, match, body, user) {
    var task = db
      .prepare(
        `SELECT t.*,p.student_id FROM tasks t JOIN plans p ON p.id=t.plan_id WHERE t.id=?`,
      )
      .get(match[1]);
    if (!task) return fail(res, 404, "NOT_FOUND", "فعالیت پیدا نشد.");
    if (Object.prototype.hasOwnProperty.call(body, "examId") && body.examId) {
      var exid = str(body.examId, 120);
      if (
        !db
          .prepare(
            "SELECT id FROM exams WHERE id=? AND (student_id=? OR student_id IS NULL)",
          )
          .get(exid, task.student_id)
      )
        return fail(res, 400, "VALIDATION", "آزمون مرتبط معتبر نیست.");
    }
    var map = {
        ...
        var col = map[k],
          val =
            col === "test_count" || col === "sort_order"
              ? Math.max(0, num(body[k], 0))
              : str(body[k], 1000);
        if ((col === "start_time" || col === "end_time") && !timeValid(val))
          return;
        db.prepare(
          "UPDATE tasks SET " + col + "=?,updated_at=? WHERE id=?",
        ).run(val || null, now(), task.id);
      }
    });
    audit(user, "update", "task", task.id, body);
    var p = db.prepare("SELECT * FROM plans WHERE id=?").get(task.plan_id);
    ok(res, mapPlan(p, task.student_id));
  },
);
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var r = db.prepare("DELETE FROM tasks WHERE id=?").run(match[1]);
    if (!r.changes) return fail(res, 404, "NOT_FOUND", "فعالیت پیدا نشد.");
...
router.add(
  ...
  ["admin"],
  function (req, res) {
    ok(res, db.prepare("SELECT * FROM subjects ORDER BY display_order").all());
  },
);
router.add(
  "POST",
...
router.add(
  ...
  function (req, res, match, body, user) {
    var title = str(body.title, 200),
      iso = str(body.isoDate, 10),
      pd = str(body.persianDate, 100),
      sid = str(body.studentId, 120);
    if (!title || !isoDateValid(iso) || !pd || !sid)
      return fail(
        res,
        400,
        "VALIDATION",
        "دانش‌آموز، عنوان، تاریخ شمسی و تاریخ ISO لازم است.",
      );
    if (!db.prepare("SELECT id FROM students WHERE id=?").get(sid))
      return fail(res, 404, "NOT_FOUND", "دانش‌آموز پیدا نشد.");
    var openAt = str(body.openAt, 80) || iso + "T08:00:00+03:30",
      closeAt = str(body.closeAt, 80) || iso + "T13:00:00+03:30";
    if (
      !dateTimeValid(openAt) ||
      !dateTimeValid(closeAt) ||
      new Date(closeAt) <= new Date(openAt)
    )
      return fail(res, 400, "VALIDATION", "بازه زمانی آزمون معتبر نیست.");
    var id = security.id("exam"),
      t = now();
    db.prepare(
      "INSERT INTO exams (id,title,persian_date,iso_date,note,status,created_at,updated_at,student_id,open_at,close_at,duration_minutes,max_attempts,instructions,published) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    ).run(
    ...
    audit(user, "create", "exam", id, body);
    ok(
      res,
      mapExam(db.prepare("SELECT * FROM exams WHERE id=?").get(id), sid, true),
      201,
    );
  },
);
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var e = db.prepare("SELECT * FROM exams WHERE id=?").get(match[1]);
    if (!e) return fail(res, 404, "NOT_FOUND", "آزمون پیدا نشد.");
    var title = Object.prototype.hasOwnProperty.call(body, "title")
      ? str(body.title, 200)
      : e.title;
    var pd = Object.prototype.hasOwnProperty.call(body, "persianDate")
      ? str(body.persianDate, 100)
      : e.persian_date;
    var iso = Object.prototype.hasOwnProperty.call(body, "isoDate")
      ? str(body.isoDate, 10)
      : e.iso_date;
    var sid = Object.prototype.hasOwnProperty.call(body, "studentId")
    ...
    var closeAt = Object.prototype.hasOwnProperty.call(body, "closeAt")
      ? str(body.closeAt, 80)
      : examDefaultClose(e);
    if (!title || !pd || !isoDateValid(iso) || !sid)
      return fail(
        res,
        400,
        "VALIDATION",
    ...
    var duration = Object.prototype.hasOwnProperty.call(body, "durationMinutes")
      ? Math.max(1, num(body.durationMinutes, 120))
      : Math.max(1, Number(e.duration_minutes || 120));
    var published = Object.prototype.hasOwnProperty.call(body, "published")
      ? boolInt(body.published)
      : Number(e.published == null ? 1 : e.published);
    var note = Object.prototype.hasOwnProperty.call(body, "note")
      ? str(body.note, 1000)
      : e.note || "";
    var status = Object.prototype.hasOwnProperty.call(body, "status")
      ? str(body.status, 30)
      : e.status || "upcoming";
    var instructions = Object.prototype.hasOwnProperty.call(
      body,
      "instructions",
    )
      ? str(body.instructions, 3000)
    ...
    if (qz)
      db.prepare(
        "UPDATE quizzes SET title=?,duration_minutes=?,updated_at=? WHERE id=?",
      ).run(title, duration, t, qz.id);
    audit(user, "update", "exam", e.id, body);
    var fresh = db.prepare("SELECT * FROM exams WHERE id=?").get(e.id);
    ok(res, mapExam(fresh, sid, true));
  },
);
router.add(
  "DELETE",
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var r = db.prepare("DELETE FROM exams WHERE id=?").run(match[1]);
    if (!r.changes) return fail(res, 404, "NOT_FOUND", "آزمون پیدا نشد.");
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    if (!db.prepare("SELECT id FROM exams WHERE id=?").get(match[1]))
      return fail(res, 404, "NOT_FOUND", "آزمون پیدا نشد.");
    var subject = str(body.subject, 150),
      desc = str(body.description, 1000);
    if (!subject || !desc)
      return fail(res, 400, "VALIDATION", "درس و توضیح بودجه لازم است.");
    var id = security.id("syllabus");
    db.prepare(
      "INSERT INTO exam_syllabus (id,exam_id,subject_label,description,required,track) VALUES (?,?,?,?,?,?)",
    ).run(
      id,
      ...
      subject,
      desc,
      boolInt(body.required),
      str(body.track, 100),
    );
    audit(user, "create", "exam_syllabus", id, body);
    var ee = db.prepare("SELECT * FROM exams WHERE id=?").get(match[1]);
    ok(res, mapExam(ee, ee.student_id || "", true), 201);
  },
);
router.add(
  "DELETE",
  ...
  ["admin"],
  function (req, res, match, body, user) {
    db.prepare("DELETE FROM exam_syllabus WHERE id=?").run(match[1]);
  ...
  function (req, res, match, body, user) {
    ...
    if (!qz) {
      ...
      db.prepare(
        ...
        e.title,
        "آزمون اصلی",
        Math.max(1, num(e.duration_minutes, 120)),
        e.id,
        t,
        t,
      );
      qz = db.prepare("SELECT * FROM quizzes WHERE id=?").get(qid);
    }
    ...
    if (
      ...
      return fail(
        ...
        400,
        "VALIDATION",
        "صورت سؤال، چهار گزینه و پاسخ صحیح لازم است.",
      );
    var id = security.id("question"),
      sortOrder = num(body.sortOrder, Number(qz.question_count || 0) + 1);
    db.prepare(
      "INSERT INTO quiz_questions (id,quiz_id,question_text,option_a,option_b,option_c,option_d,correct_option,explanation,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?)",
    ).run(
      id,
    ...
    audit(user, "create", "exam_question", id, { examId: e.id });
    ok(res, { id: id }, 201);
  },
);
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    if (!qz) return fail(res, 404, "NOT_FOUND", "آزمون پیدا نشد.");
    var r = db
      .prepare("DELETE FROM quiz_questions WHERE id=? AND quiz_id=?")
      .run(match[2], qz.id);
    if (!r.changes) return fail(res, 404, "NOT_FOUND", "سؤال پیدا نشد.");
...
router.add(
  ...
  function (req, res) {
    var q = query(req),
      sid = str(q.studentId, 120),
      sql = `SELECT r.*,e.title AS examTitle,s.name AS studentName FROM exam_attempt_requests r JOIN exams e ON e.id=r.exam_id JOIN students s ON s.id=r.student_id WHERE 1=1`,
      args = [];
    if (sid) {
...
router.add(
  ...
  function (req, res, match, body, user) {
    var r = db
      .prepare("SELECT * FROM exam_attempt_requests WHERE id=?")
      .get(match[1]);
    if (!r) return fail(res, 404, "NOT_FOUND", "درخواست پیدا نشد.");
    var status =
      ["approved", "rejected"].indexOf(body.status) >= 0 ? body.status : null;
    if (!status) return fail(res, 400, "VALIDATION", "وضعیت معتبر نیست.");
    var t = now();
    db.prepare(
      "UPDATE exam_attempt_requests SET status=?,advisor_note=?,updated_at=?,resolved_by=?,resolved_at=? WHERE id=?",
    ).run(status, str(body.advisorNote, 1200), t, user.id, t, r.id);
    var e = db.prepare("SELECT title FROM exams WHERE id=?").get(r.exam_id);
    notifyStudent(
      r.student_id,
      status === "approved"
        ? "تلاش مجدد آزمون تأیید شد"
        : "درخواست تلاش مجدد بررسی شد",
      status === "approved"
        ? "برای آزمون «" +
            (e ? e.title : "") +
            "» یک تلاش اضافه تا ۲۴ ساعت فعال شد."
        : str(body.advisorNote, 1000) || "درخواست شما تأیید نشد.",
    );
    emitStudent(r.student_id, "exam.retry_reviewed", {
      examId: r.exam_id,
      requestId: r.id,
      status: status,
      resolvedAt: t,
    });
    audit(user, "review", "exam_attempt_request", r.id, { status: status });
    ok(res, { id: r.id, status: status, resolvedAt: t });
  },
);
...
router.add(
  ...
  function (req, res, match, body, user) {
    var title = str(body.title, 200);
    if (!title) return fail(res, 400, "VALIDATION", "عنوان آزمون لازم است.");
    var id = security.id("quiz"),
      t = now();
    db.prepare(
    ...
    audit(user, "create", "quiz", id, body);
    ok(res, { id: id, title: title }, 201);
  },
);
router.add(
  "PATCH",
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var qz = db.prepare("SELECT * FROM quizzes WHERE id=?").get(match[1]);
    if (!qz) return fail(res, 404, "NOT_FOUND", "آزمونک پیدا نشد.");
    var map = {
      title: "title",
      subject: "subject",
    ...
    Object.keys(map).forEach(function (k) {
      if (Object.prototype.hasOwnProperty.call(body, k)) {
        var val =
          k === "durationMinutes"
            ? Math.max(1, num(body[k], 20))
            : k === "active"
              ? boolInt(body[k])
              : str(body[k], 200) || null;
        db.prepare(
          "UPDATE quizzes SET " + map[k] + "=?,updated_at=? WHERE id=?",
        ).run(val, now(), qz.id);
      }
    });
    audit(user, "update", "quiz", qz.id, body);
    ok(res, db.prepare("SELECT * FROM quizzes WHERE id=?").get(qz.id));
  },
);
router.add(
  "GET",
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    if (!db.prepare("SELECT id FROM quizzes WHERE id=?").get(match[1]))
      return fail(res, 404, "NOT_FOUND", "آزمونک پیدا نشد.");
    ...
    if (
      ...
      return fail(
        ...
        400,
        "VALIDATION",
        "سؤال، چهار گزینه و پاسخ صحیح لازم است.",
      );
    var id = security.id("question");
    db.prepare(
      "INSERT INTO quiz_questions (id,quiz_id,question_text,option_a,option_b,option_c,option_d,correct_option,explanation,sort_order) VALUES (?,?,?,?,?,?,?,?,?,?)",
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    db.prepare("DELETE FROM quiz_questions WHERE id=?").run(match[1]);
...
router.add("GET", /^\/api\/v1\/admin\/live$/, ["admin"], function (req, res) {
  ...
  ok(res, {
    student: student,
    presence: getPresence(sid),
    activeSession: activeStudySession(sid),
...
router.add(
  ...
  function (req, res, match, body, user) {
    ...
    ok(res, { updated: r.changes, published: !!published });
  },
);
router.add(
  "GET",
  ...
  function (req, res) {
    var q = query(req),
      sid = str(q.studentId, 120) || "student_sister",
...
router.add(
  ...
  ["admin"],
  function (req, res, match, body, user) {
    var normalized = normalizeImportPayload(
      body.data || body,
      body.studentId || "",
    );
    ok(res, normalized);
  ...
  function (req, res, match, body, user) {
    ...
    try {
      ...
      if (
        ...
        return fail(res, 409, "IMPORT_CONFLICT", String(e.message));
      throw e;
    }
  },
);
...
router.add("GET", /^\/api\/v1\/admin\/audit$/, ["admin"], function (req, res) {
  ok(
    res,
    db
      .prepare(
...
server.listen(env.port, "0.0.0.0", function () {
  ...
  if (env.nodeEnv !== "production") {
    ...
    console.log(
      "Dev student: " + env.studentUsername + " / " + env.studentPassword,
    );
  }
});
