...
export function ActivityIsland({ syncLabel }: { syncLabel: string }) {
  ...
  const activeExam = exams.find(
    (item) =>
      Boolean(item.delivery?.activeAttemptId) ||
      item.delivery?.state === "active",
  );
  ...
  const audioActive = Boolean(
    selectedTrack && !["idle", "ended", "error"].includes(playback),
  );
  const showAudio =
    audioActive &&
      ...
      [
        ...
        session
          ? {
              kind: "study",
              label: overtime
                ? "بیشتر از زمان برنامه"
                : session.status === "running"
                  ? "جلسه مطالعه"
                  : "مطالعه در مکث",
              title:
                [task?.subject, task?.title].filter(Boolean).join(" · ") ||
                "فعالیت جاری",
              meta: overtime
                ? `+${formatTime(elapsed - taskLimitSeconds)}`
                : formatTime(elapsed),
            }
          : null,
        showAudio && selectedTrack
          ? {
              kind: "audio",
              label: audioBuffering
  ...
  useEffect(() => {
    if (session?.status !== "running" || session.id.startsWith("local-"))
      return;
  ...
  return (
    <aside
      ref={root}
      className={`activity-island ${expanded ? "is-expanded" : ""} ${activities.length ? "has-activities" : "is-idle"} ${overtime ? "has-overtime" : ""}`}
      aria-label="فعالیت‌های در حال اجرا"
      aria-live="polite"
    >
      <div className="activity-island__compact">
        <button
          ref={summaryButton}
          type="button"
          className="activity-island__summary"
          onClick={() => activities.length && setExpanded((value) => !value)}
          aria-expanded={expanded}
          aria-controls="activity-island-sessions"
          disabled={!activities.length}
        >
          <span
            className={`activity-island__primary-icon activity-island__primary-icon--${primary?.kind || "idle"}`}
            aria-hidden="true"
          >
            {primary
              ? activityIcon(
                  primary.kind,
                  primary.kind === "sync" && syncLabel !== "همگام شد",
                )
              : null}
          </span>
          <span className="activity-island__summary-copy">
            <small>{primary?.label || "فعالیت جاری"}</small>
            <strong>
              {primary?.title || "اکنون فعالیتی در حال اجرا نیست"}
            </strong>
          </span>
          {primary?.meta ? (
            <b className="activity-island__time" dir="ltr">
              {primary.meta}
            </b>
          ) : null}
          <span
            className="activity-island__markers"
            aria-label={`${activities.length.toLocaleString("fa-IR")} فعالیت جاری`}
          >
            {activities.slice(0, 3).map((activity) => (
              <i
                key={activity.kind}
                className={`activity-marker activity-marker--${activity.kind}`}
                title={activity.label}
              />
            ))}
            {activities.length > 3 ? (
              <em>+{(activities.length - 3).toLocaleString("fa-IR")}</em>
            ) : null}
          </span>
          {activities.length ? (
            <ChevronUp
              className="activity-island__chevron"
              aria-hidden="true"
            />
          ) : null}
        </button>
        {primaryHasControl ? (
          <button
            type="button"
            className={`activity-island__quick-control activity-island__quick-control--${primary.kind}`}
            onClick={() =>
              void (primary.kind === "study"
                ? session?.status === "running"
                  ? pause()
                  : resume()
                : toggleAudio())
      ...
      {showAudio && audioBuffering ? (
        <span
          className="activity-island__buffer-track"
          aria-label={`آماده‌سازی صوت ${Math.round(bufferedPercent).toLocaleString("fa-IR")} درصد`}
        >
          <i style={{ inlineSize: `${Math.max(4, bufferedPercent)}%` }} />
        </span>
      ) : null}
      {expanded ? (
        <div className="activity-island__board" id="activity-island-sessions">
          <header>
            <span>
              <small>مرکز فعالیت</small>
              <strong>
                {activities.length.toLocaleString("fa-IR")} فعالیت جاری
              </strong>
            </span>
            <button
              type="button"
          ...
          <div className="activity-island__sessions">
            {activeExam ? (
              <Session
                ...
                meta="ادامه آزمون"
                onOpen={() =>
                  go(`/exam?exam=${encodeURIComponent(activeExam.id)}`)
                }
              />
            ) : null}
            {session ? (
              <Session
                ...
                onOpen={() =>
                  go(`/plan?task=${encodeURIComponent(session.taskId)}`)
                }
                control={
                  <button
                    type="button"
                    onClick={() =>
                      void (session.status === "running" ? pause() : resume())
                    }
                    aria-label={
                      session.status === "running"
                        ? "مکث مطالعه"
            ...
            {showAudio && selectedTrack ? (
              <Session
                kind="audio"
                icon={audioBuffering ? <LoaderCircle /> : <Music2 />}
                label={
                  audioBuffering
                    ? "در حال آماده‌سازی صوت"
                    : audioPlaying
                ...
                onOpen={() => go("/more/audio")}
                control={
                  <button
                    type="button"
                    onClick={() => void toggleAudio()}
                    aria-label={
                      audioPlaying || audioBuffering ? "مکث صوت" : "ادامه صوت"
                    }
                  >
                    {audioBuffering ? (
                      <LoaderCircle />
            ...
            {syncLabel !== "آنلاین" ? (
              <Session
                ...
                label="وضعیت داده‌ها"
                title={syncLabel}
                onOpen={() => go("/more#sync-status")}
                control={
                  <span aria-label={syncLabel}>
                    {syncLabel === "همگام شد" ? <Check /> : <LoaderCircle />}
                  </span>
                }
              />
            ) : null}
...
function Session({
  ...
  return (
    <section
      className={`island-session island-session--${kind} ${overdue ? "is-overdue" : ""}`}
    >
      <button
        type="button"
        className="island-session__link"
        onClick={onOpen}
        aria-label={`${label}: ${title}`}
      >
        <span className="island-session__icon" aria-hidden="true">
          {icon}
        </span>
        <span className="island-session__copy">
          <small>{label}</small>
          <strong>{title}</strong>
          {meta ? <b dir="ltr">{meta}</b> : null}
        </span>
        <ChevronLeft aria-hidden="true" />
      </button>
      {control ? (
        <span className="island-session__control">{control}</span>
      ) : null}
    </section>
  );
}
