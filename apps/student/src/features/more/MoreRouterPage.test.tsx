...
describe("More navigation hub", () => {
  it("puts adviser chat first and exposes dedicated section routes", () => {
    ...
    renderPage("/more");
    const hub = screen
      .getByRole("link", { name: /گفت‌وگو با مشاور/ })
      .closest("section")!;
    expect(
      within(hub).getByRole("link", { name: /گفت‌وگو با مشاور/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: /صوت‌ها و آرامش/ }),
    ).toHaveAttribute("href", "/more/audio");
    expect(screen.getByRole("link", { name: /تنظیمات/ })).toHaveAttribute(
      "href",
      "/more/settings",
    );
    expect(
      screen.getByRole("link", { name: /روند و پیشنهادها/ }),
    ).toHaveAttribute("href", "/more/insights");
    expect(
      screen.getByRole("link", { name: /دفترچه اشتباهات/ }),
    ).toHaveAttribute("href", "/more/mistakes");
    expect(screen.getByRole("link", { name: /انتخاب سرپرست/ })).toHaveAttribute(
      "href",
      "/more/guardian",
    );
    expect(
      screen.getByRole("link", { name: /کتاب‌های درسی من/ }),
    ).toHaveAttribute("href", "/more/books");
  });

  it("shows books selected from the student account education profile", async () => {
    useStudentStore.setState({
      access: portalAccess(["STUDENT"], ["student.profile.read"]),
    } as never);
    ...
    renderPage("/more/books");
    expect(await screen.findByText("زیست‌شناسی ۳")).toBeInTheDocument();
    expect(screen.getByText(/پایه دوازدهم/)).toBeInTheDocument();
    expect(screen.getAllByText(/علوم تجربی/)).not.toHaveLength(0);
    expect(apiClient.request).toHaveBeenCalledWith(
      "GET",
      "/education-catalog/my-books",
    );
  });
  ...
  it("lets a student explicitly enable guardian read-only chat access", async () => {
    ...
    const request = vi
      ...
      .mockImplementation(async (method, path) => {
        if (method === "GET" && path === "/student/chat-privacy")
          return { guardianReadOnly: false } as never;
        if (method === "PATCH" && path === "/student/chat-privacy")
          return { guardianReadOnly: true } as never;
        if (path === "/auth/sessions") return [] as never;
        return {} as never;
      });
    renderPage("/more/settings");
    const toggle = await screen.findByRole("checkbox", {
      name: "نمایش فقط‌خواندنی گفتگوها به سرپرست",
    });
    await userEvent.click(toggle);
    expect(request).toHaveBeenCalledWith("PATCH", "/student/chat-privacy", {
      guardianReadOnly: true,
    });
  ...
  it("edits the Telegram-style chat profile from More", async () => {
    const request = vi
      .spyOn(apiClient, "request")
      .mockImplementation(async (method, path) => {
        if (method === "GET" && path === "/chat/profile")
          return {
            id: "u1",
            username: "sara",
            displayName: "سارا",
            bio: "",
            avatarUrl: "",
            usernameChange: { count: 0, allowed: true },
          } as never;
        if (method === "PATCH" && path === "/chat/profile")
          return {
            id: "u1",
            username: "sara_k",
            ...
            bio: "کنکوری ۱۴۰۶",
            avatarUrl: "",
            usernameChange: { count: 1, allowed: false },
          } as never;
        return {} as never;
      });
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "ذخیره پروفایل" }),
    );
  ...
  it("searches for a guardian and submits a pending selection", async () => {
    useStudentStore.setState({
      access: portalAccess(
        ["STUDENT"],
        ["student.profile.read", "tasks.update", "learning.create"],
      ),
    } as never);
    const request = vi
      .spyOn(apiClient, "request")
      .mockImplementation(async (method, path) => {
        if (method === "GET" && path === "/student/guardian-selection")
          return {
            relationships: [],
            change: { allowed: true, nextAllowedAt: null },
          } as never;
        if (method === "GET" && path.startsWith("/student/guardian-candidates"))
          return [
            {
              id: "g1",
              username: "parent.sara",
              name: "مریم کریمی",
              organization: { id: "o1", name: "مدرسه امید" },
            },
          ] as never;
        if (method === "POST" && path === "/student/guardian-selection")
          return { id: "r1", status: "PENDING" } as never;
        return {} as never;
      });
  ...
  it("shows the selected child subject plan without student-only private domains", async () => {
    useStudentStore.setState({
      access: portalAccess(
        ["GUARDIAN"],
        ["guardian.students.read", "studentSubjects.read"],
      ),
    ...
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        <Route
          path="/more/:section?"
          element={
            <MoreRouterPage theme="light" onThemeChange={onThemeChange} />
          }
        />
      </Routes>
    </MemoryRouter>,
