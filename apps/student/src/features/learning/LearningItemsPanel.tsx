...
export function LearningItemsPanel({ canCreate, canUpdate, canReview }: { canCreate: boolean; canUpdate: boolean; canReview: boolean }) {
  ...
  return <section className="learning-items" aria-labelledby="learning-items-title">
    ...
    <div className="learning-items__list">{items.filter((item) => item.status !== 'archived').map((item) => <article key={item.id} className={item.status === 'done' ? 'is-done' : ''}><div><strong>{item.title}</strong><small>{[item.subject, item.dueDate ? formatDate(item.dueDate) : ''].filter(Boolean).join(' · ')}</small>{item.note ? <p>{item.note}</p> : null}</div>{canReview ? <div className="learning-items__rating" aria-label={`امتیاز یادگیری ${item.title}`}>{[1, 2, 3, 4, 5].map((rating) => <button type="button" key={rating} disabled={status === 'saving'} onClick={() => void review(item, rating)} aria-label={`امتیاز ${rating}`}><Star className={rating <= Number(item.mastery || 0) ? 'is-active' : ''} aria-hidden="true" /></button>)}</div> : null}{canUpdate ? <div className="learning-items__actions"><button type="button" disabled={status === 'saving'} onClick={() => void update(item, { status: item.status === 'done' ? 'pending' : 'done' })}>{item.status === 'done' ? <RotateCcw aria-hidden="true" /> : <CheckCircle2 aria-hidden="true" />}{item.status === 'done' ? 'بازکردن' : 'انجام شد'}</button><button type="button" className="is-danger" disabled={status === 'saving'} onClick={() => void remove(item)}><Trash2 aria-hidden="true" />حذف</button></div> : null}</article>)}{status === 'ready' && !items.length ? <p className="settings-empty">هنوز مرور شخصی نساخته‌اید.</p> : null}</div>
  </section>;
}
