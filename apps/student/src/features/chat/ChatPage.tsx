...
export function ChatPage() {
  ...
  const [status, setStatus] = useState<
    "loading" | "ready" | "sending" | "error"
  >("loading");
  const [online, setOnline] = useState(navigator.onLine);
  const [conversation, setConversation] = useState<Conversation | null>(null);
  const [conversations, setConversations] = useState<Conversation[]>([]);
  const [inboxStatus, setInboxStatus] = useState<"loading" | "ready" | "error">(
    "loading",
  );
  ...
  async function openPeerProfile() {
    ...
    setProfile(conversation.peer);
    setProfileOpen(true);
    try {
      setProfile(
        await apiClient.request<ConversationParticipant>(
          "GET",
          `/chat/profiles/${encodeURIComponent(conversation.peer.id)}`,
        ),
      );
    } catch {
  ...
  async function loadOlder() {
    ...
    try {
      const older = await apiClient.request<ChatMessage[]>(
        "GET",
        `/chat/conversations/${encodeURIComponent(conversation.id)}/messages?limit=${PAGE_SIZE}&before=${encodeURIComponent(first.createdAt)}`,
      );
  ...
  function markChatRead(nextMessages = messages) {
    ...
    if (conversation && !conversation.readOnly)
      void apiClient
        .request(
          "POST",
          `/chat/conversations/${encodeURIComponent(conversation.id)}/read`,
        )
        .catch(() => undefined);
  }
  ...
  async function removeMessage(message: ChatMessage) {
    if (!conversation || !isMine(message) || message.id.startsWith("pending-"))
      return;
    await apiClient.request(
      "DELETE",
      `/chat/conversations/${encodeURIComponent(conversation.id)}/messages/${encodeURIComponent(message.id)}`,
    );
    setMessages((current) =>
      current.map((item) =>
        item.id === message.id
  ...
  async function reactWith(message: ChatMessage, emoji: string) {
    ...
    await apiClient.request(
      "POST",
      `/chat/conversations/${encodeURIComponent(conversation.id)}/messages/${encodeURIComponent(message.id)}/reactions`,
      { emoji },
    );
    setActiveMessageId(null);
  }
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void loadConversations(true);
      if (conversation) void openConversation(conversation, true);
    }, 15_000);
    const realtime = (event: Event) => {
      if (
        (event as CustomEvent<{ type?: string }>).detail?.type?.startsWith(
          "chat.",
        )
      ) {
        void loadConversations(true);
  ...
  const unreadCount = messages.filter(
    (message) =>
      !isMine(message) &&
      (!lastReadAt || new Date(message.createdAt || 0) > new Date(lastReadAt)),
  ).length;
    ...
    return (
      <section className="student-chat-inbox" aria-label="گفت‌وگوها">
        ...
        <div className="student-chat-inbox__list">
          ...
          {inboxStatus === "ready" && !visibleConversations.length ? (
            <div className="student-chat-inbox__empty">
              <MessageCircle />
              <strong>
                {inboxSearch ? "گفتگویی پیدا نشد" : "هنوز گفتگویی ندارید"}
              </strong>
              <small>
                {inboxSearch
    ...
    >
      ...
      {profileOpen && profile ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`پروفایل ${profile.displayName || profile.name || profile.username || "کاربر"}`}
        >
          <button
            type="button"
          ...
          <article>
            ...
            <h2>{profile.displayName || profile.name || profile.username}</h2>
      ...
      >
        ...
        {status !== "loading" && !visibleMessages.length ? (
          <div className="student-chat__state">
          ...
          return (
            <div key={message.id}>
              ...
              <Bubble
                ...
                onReact={(emoji) => void reactWith(message, emoji)}
              />
            </div>
          );
        })}
      ...
      {!conversation.readOnly ? (
        ...
        >
          ...
          <button
            type="submit"
            disabled={!text.trim() || status === "sending" || !conversation}
            aria-label="ارسال پیام"
          >
            {status === "sending" ? <LoaderCircle /> : <Send />}
          </button>
        </form>
      ) : null}
    </section>
...
function statusLabel(status: string, online: boolean) {
