...
export class ExamsService {
  constructor(
    ...
    private readonly assignments?: Repository<ExamAssignment>,
    @Optional()
  ...
  async listScoped(
    ...
    const exams = await this.exams.find({
      where: platform ? {} : { organization: { id: In(organizationIds) } },
      relations: {
        questions: true,
        organization: true,
        assignments: true,
  ...
  async organizationIdForExam(id: string) {
    const exam = await this.exams.findOneOrFail({
      where: { id },
  ...
  async examIdForQuestion(id: string) {
    const question = await this.questions.findOneOrFail({
      where: { id },
      relations: { exam: true },
    });
  ...
  async progress(examId: string, userId: string) {
    ...
    if (attempt && this.isExpired(attempt))
      return this.submit(attempt.id, [], userId);
  ...
  async saveProgress(
    ...
    if (!attempt)
      throw new ApiException(404, "ATTEMPT_NOT_FOUND", "تلاش آزمون پیدا نشد.");
    if (attempt.finishedAt)
      throw new ApiException(
        409,
        "EXAM_ALREADY_SUBMITTED",
        "این آزمون قبلاً ثبت نهایی شده است.",
      );
    ...
    const questionIds = new Set(
      attempt.exam.questions.map((question) => question.id),
    );
    if (answers.some((answer) => !questionIds.has(answer.questionId)))
      throw new ApiException(
        400,
        "QUESTION_NOT_IN_EXAM",
        "یکی از سؤال‌ها متعلق به این آزمون نیست.",
      );
  ...
  async create(
    ...
    const [organization, createdBy] = await Promise.all([
      dto.organizationId
        ? this.exams.manager.findOneBy(Organization, { id: dto.organizationId })
        : Promise.resolve(null),
      actorUserId
        ? this.exams.manager.findOneBy(User, { id: actorUserId })
        : Promise.resolve(null),
    ]);
    if (dto.organizationId && !organization)
      throw new ApiException(404, "ORGANIZATION_NOT_FOUND", "سازمان یافت نشد.");
    const exam = this.exams.create({
      ...
      lifecycleStatus:
        dto.lifecycleStatus || (dto.published ? "scheduled" : "draft"),
      navigationMode:
        dto.navigationMode ||
        (dto.allowBackNavigation === false ? "sequential" : "free"),
      ...
      startTime:
        dto.startTime || dto.openAt
          ? new Date(dto.startTime || dto.openAt || "")
          : null,
      endTime:
        dto.endTime || dto.closeAt
          ? new Date(dto.endTime || dto.closeAt || "")
          : null,
      questions: (dto.questions || []).map((question) =>
        this.questions.create(this.normalizeQuestion(question)),
      ),
      organization,
      createdBy,
    });
    ...
    return this.publicExam(
      await this.exams.findOneOrFail({
        where: { id: saved.id },
  ...
  async update(id: string, body: Record<string, unknown>) {
    ...
    if (attempts !== undefined)
      exam.attemptLimit = Math.max(1, Number(attempts));
    if (typeof body.published === "boolean") exam.published = body.published;
    if (
      ["konkur", "mock", "practice", "quiz", "diagnostic"].includes(
        String(body.mode),
      )
    )
      exam.mode = body.mode as Exam["mode"];
    ...
    if (
      ["immediate", "scheduled", "manual"].includes(String(body.resultPolicy))
    )
    ...
    if (body.answerKeyReleaseAt !== undefined)
      exam.answerKeyReleaseAt = nullableDate(body.answerKeyReleaseAt);
    if (body.explanationReleaseAt !== undefined)
      exam.explanationReleaseAt = nullableDate(body.explanationReleaseAt);
    if (body.rankingReleaseAt !== undefined)
      exam.rankingReleaseAt = nullableDate(body.rankingReleaseAt);
    if (body.latestStartAt !== undefined)
      exam.latestStartAt = nullableDate(body.latestStartAt);
    if (
      ["free", "section_only", "sequential"].includes(
        String(body.navigationMode),
      )
    )
      exam.navigationMode = body.navigationMode as Exam["navigationMode"];
    if (["whole_exam", "per_section"].includes(String(body.timerMode)))
      exam.timerMode = body.timerMode as Exam["timerMode"];
    for (const key of [
      "allowResume",
      "allowLateStart",
      "allowPracticeAfterDeadline",
      "autoSubmitOnTimeout",
      "integrityMonitoring",
    ] as const)
      if (typeof body[key] === "boolean") exam[key] = body[key] as never;
    if (typeof body.resultsReleased === "boolean")
      exam.resultsReleased = body.resultsReleased;
    if (Array.isArray(body.sections))
      exam.sections = body.sections as Exam["sections"];
  ...
  async start(examId: string, userId: string) {
    ...
    if (exam.latestStartAt && now >= exam.latestStartAt && !exam.allowLateStart)
      throw new ApiException(
        409,
        "EXAM_LATEST_START_PASSED",
        "مهلت شروع این آزمون به پایان رسیده است.",
      );
    if (!exam.questions.length)
      throw new ApiException(
        409,
        "EXAM_HAS_NO_QUESTIONS",
        "این آزمون هنوز سؤال قابل پاسخ ندارد.",
      );
    ...
    if (used >= exam.attemptLimit)
      throw new ApiException(
        ...
        "تعداد دفعات مجاز آزمون تکمیل شده است.",
      );
    const durationEnd = new Date(now.getTime() + exam.duration * 60_000);
    const expiresAt =
      exam.endTime && exam.endTime < durationEnd ? exam.endTime : durationEnd;
  ...
  async submit(
    ...
    const evaluation = (
      this.scoringEngine || new ExamScoringService()
    ).evaluate(attempt.exam, finalAnswers);
    const score = Math.round(evaluation.percentage);
    const finishedAt = new Date();
    await this.attempts.update(attemptId, {
      answers: finalAnswers,
      score,
      status: this.isExpired(attempt) ? "expired" : "submitted",
      finishedAt,
    ...
    if (this.mistakes) {
      for (const question of questions.filter(
        (item) =>
          answerMap.get(item.id) &&
          answerMap.get(item.id) !== item.correctAnswer,
      )) {
        const existing = await this.mistakes.findOne({
          where: { studentId: student.id, questionId: question.id },
        });
        if (!existing)
          await this.mistakes.save(
  ...
  async submitExam(
    ...
    if (!attempt)
      throw new ApiException(
        409,
        "NO_ACTIVE_ATTEMPT",
        "تلاش فعالی برای این آزمون وجود ندارد.",
      );
  ...
  async heartbeat(
    ...
    if (
      currentSectionId &&
      !attempt.exam.sections.some((section) => section.id === currentSectionId)
    )
      throw new ApiException(
        400,
        "SECTION_NOT_IN_EXAM",
        "دفترچه انتخاب‌شده متعلق به این آزمون نیست.",
      );
    ...
    return {
      ...
      status: "active",
      serverTime: lastHeartbeatAt.toISOString(),
      expiresAt: this.deadline(attempt).toISOString(),
    };
  }
  ...
  private result(
    ...
    const evaluation = (
      this.scoringEngine || new ExamScoringService()
    ).evaluate(attempt.exam, answers);
    const status = this.resultStatus(attempt.exam);
    const released = privileged || status === "released";
    const now = new Date();
    const answerKeyReleased =
      privileged ||
      Boolean(
        attempt.exam.answerKeyReleaseAt
          ? now >= attempt.exam.answerKeyReleaseAt
          : released,
      );
    const explanationsReleased =
      privileged ||
      Boolean(
        attempt.exam.explanationReleaseAt
          ? now >= attempt.exam.explanationReleaseAt
          : answerKeyReleased,
      );
    return {
      id: attempt.id,
      ...
      review: answerKeyReleased
        ? questions.map((question) => ({
            ...
            explanation: explanationsReleased
  ...
  private deadline(attempt: ExamAttempt) {
    const durationEnd =
      attempt.expiresAt ||
      new Date(attempt.startedAt.getTime() + attempt.exam.duration * 60_000);
    return attempt.exam.endTime && attempt.exam.endTime < durationEnd
  ...
  private attemptProgress(attempt: ExamAttempt) {
    ...
    return {
      ...
      examCloseAt: attempt.exam.endTime || null,
      deadlineAt: deadline.toISOString(),
      serverTime: new Date().toISOString(),
      finishedAt: attempt.finishedAt || null,
      savedAnswers: (attempt.answers || []).map((answer, index) => ({
        ...answer,
        clientUpdatedAt:
          answer.clientUpdatedAt || attempt.startedAt.toISOString(),
        revision: answer.revision ?? index + 1,
      })),
      ...
      navigationMode:
        attempt.exam.navigationMode ||
        (attempt.exam.allowBackNavigation ? "free" : "sequential"),
      timerMode: attempt.exam.timerMode || "whole_exam",
      currentSectionId:
        attempt.currentSectionId || attempt.exam.sections?.[0]?.id || "",
  ...
  private enforceNavigationPolicy(
    attempt: ExamAttempt,
    incoming: AttemptAnswer[],
  ) {
    if (attempt.exam.allowBackNavigation !== false) return;
    const questionIndex = new Map(
      attempt.exam.questions.map((question, index) => [question.id, index]),
    );
    const combined = [...(attempt.answers || []), ...incoming].filter(
      (answer) => answer.visited,
    );
    const furthest = combined.reduce(
      (best, answer) =>
        Math.max(best, questionIndex.get(answer.questionId) ?? -1),
      -1,
    );
    const furthestVisit = combined
      .filter(
        (answer) => (questionIndex.get(answer.questionId) ?? -1) === furthest,
      )
    ...
    const changedEarlierAnswer = incoming.some((answer) => {
      ...
      const changed =
        !previous || (answer.revision ?? 0) > (previous.revision ?? 0);
      const changedAt = Date.parse(answer.clientUpdatedAt || "");
      return (
        changed &&
        index < furthest &&
        (!Number.isFinite(changedAt) || changedAt >= furthestVisit)
      );
    });
    if (changedEarlierAnswer)
      throw new ApiException(
        409,
        "BACK_NAVIGATION_FORBIDDEN",
        "بازگشت و تغییر پاسخ سؤال‌های قبلی در این آزمون مجاز نیست.",
      );
  }
  ...
  private publicAttempt(attempt: ExamAttempt, fallbackExam?: Exam | string) {
    const exam =
      attempt.exam ||
      (typeof fallbackExam === "object" ? fallbackExam : undefined);
    const status = attempt.finishedAt ? this.resultStatus(exam) : "active";
    return {
      id: attempt.id,
      examId:
        exam?.id ??
        (typeof fallbackExam === "string" ? fallbackExam : undefined),
      title: exam?.title ?? "",
      status,
      score: status === "released" ? attempt.score : null,
      subjectSummary:
        status === "released" && exam
          ? this.subjectSummary(exam, attempt.answers || [])
          : undefined,
      startedAt: attempt.startedAt,
      finishedAt: attempt.finishedAt || null,
  ...
  private subjectSummary(exam: Exam, answers: AttemptAnswer[]) {
    ...
    return [...rows.entries()].map(([subject, row]) => ({
      subject,
      ...row,
  ...
  private publicExam(exam: Exam, includeAnswers = true, studentId?: string) {
    ...
    const latestFinished = studentAttempts.find((attempt) =>
        ...
        : beforeWindow
          ...
          : afterWindow
            ...
            : canStart
              ...
              : "closed";
    const status =
      exam.lifecycleStatus === "cancelled"
        ? "cancelled"
        : !exam.published || exam.lifecycleStatus === "draft"
          ? "draft"
          : activeAttempt
            ? "live"
            : latestFinished
              ? this.resultStatus(exam) === "released"
                ? "result_available"
                : "result_pending"
              : beforeWindow
                ? "scheduled"
    ...
    return {
      ...
      subjects: [
        ...new Set(
          [
            exam.subject,
            ...(exam.questions || []).map((question) => question.subject),
          ].filter(Boolean),
        ),
      ],
      organization: exam.organization
        ? { id: exam.organization.id, name: exam.organization.name }
        : null,
      createdAt: exam.createdAt?.toISOString(),
      updatedAt: exam.updatedAt?.toISOString(),
      mode: exam.mode === "standard" ? "mock" : exam.mode,
      status,
      instructions: exam.instructions || [],
      allowBackNavigation: exam.allowBackNavigation ?? true,
      navigationMode:
        exam.navigationMode ||
        (exam.allowBackNavigation ? "free" : "sequential"),
      timerMode: exam.timerMode || "whole_exam",
      allowResume: exam.allowResume ?? true,
      allowLateStart: exam.allowLateStart ?? false,
      allowPracticeAfterDeadline: exam.allowPracticeAfterDeadline ?? false,
      autoSubmitOnTimeout: exam.autoSubmitOnTimeout ?? true,
      sessionPolicy: exam.sessionPolicy || "allow_resume",
      integrityMonitoring: exam.integrityMonitoring ?? false,
      scoring: this.normalizeScoring(exam.scoring),
      resultPolicy: exam.resultPolicy || "immediate",
      resultReleaseAt: exam.resultReleaseAt?.toISOString() || null,
      answerKeyReleaseAt: exam.answerKeyReleaseAt?.toISOString() || null,
      explanationReleaseAt: exam.explanationReleaseAt?.toISOString() || null,
      ...
      availableFrom: exam.startTime?.toISOString(),
      availableUntil: exam.endTime?.toISOString(),
      latestStartAt: exam.latestStartAt?.toISOString() || null,
      ...
      delivery: {
        ...
        assignmentCount: exam.assignments?.length || 0,
        attemptCount: exam.attempts?.length || 0,
        activeAttemptCount: (exam.attempts || []).filter(
          (attempt) => !attempt.finishedAt,
        ).length,
        completedAttemptCount: (exam.attempts || []).filter((attempt) =>
          Boolean(attempt.finishedAt),
        ).length,
        notStartedCount: Math.max(
          ...
          (exam.assignments?.length || 0) -
            new Set(
              (exam.attempts || [])
                .map((attempt) => attempt.student?.id)
                .filter(Boolean),
            ).size,
        ),
        ...
        reason,
        state,
        lastAttempt: studentAttempts[0]
          ? this.publicAttempt(studentAttempts[0], exam)
          : null,
      },
    };
  }
  ...
  async addQuestion(examId: string, question: QuestionInput) {
    ...
    const saved = await this.questions.save(
      this.questions.create({ ...this.normalizeQuestion(question), exam }),
    );
  ...
  async updateQuestion(id: string, body: Record<string, unknown>) {
    ...
    if (Array.isArray(body.options))
      question.options = body.options.map(String);
    if (
      typeof body.correctAnswer === "string" ||
      typeof body.correctOption === "string"
    )
  ...
  async assign(examId: string, studentIds: string[], actorUserId: string) {
    if (!this.assignments || !this.users || !this.dataSource)
      throw new ApiException(
        503,
        "ASSIGNMENTS_UNAVAILABLE",
        "تخصیص آزمون در دسترس نیست.",
      );
    return this.dataSource.transaction(async (manager) => {
      ...
      if (!exam || !actor || students.length !== new Set(studentIds).size)
        throw new ApiException(
          404,
          "NOT_FOUND",
          "آزمون، کاربر یا دانش‌آموز یافت نشد.",
        );
      for (const student of students) {
        const exists = await manager.findOne(ExamAssignment, {
          where: { exam: { id: examId }, student: { id: student.id } },
        });
        if (!exists)
          await manager.save(
  ...
  async assignmentsForExam(examId: string) {
    if (!this.assignments)
      throw new ApiException(
        503,
        "ASSIGNMENTS_UNAVAILABLE",
    ...
    const assignments = await this.assignments.find({
      ...
      relations: { student: true, assignedBy: true },
        ...
        ? {
            ...
            name:
              [assignment.assignedBy.firstName, assignment.assignedBy.lastName]
                .filter(Boolean)
                .join(" ") || assignment.assignedBy.username,
          }
        : null,
    }));
  }
  ...
  async unassign(examId: string, studentId: string) {
    ...
    const started = await this.attempts.exist({
      where: { exam: { id: examId }, student: { id: studentId } },
    });
    if (started)
      throw new ApiException(
        409,
        "EXAM_ASSIGNMENT_STARTED",
        "پس از شروع آزمون، حذف تخصیص مجاز نیست.",
      );
    ...
    if (!result.affected)
      throw new ApiException(404, "NOT_FOUND", "تخصیص آزمون یافت نشد.");
  ...
  private mergeAnswers(current: AttemptAnswer[], incoming: AttemptAnswer[]) {
    const merged = new Map(
      current.map((answer) => [answer.questionId, answer]),
    );
  ...
  async resultForStudent(attemptId: string, userId: string) {
    ...
    if (!attempt.finishedAt)
      throw new ApiException(
        409,
        "ATTEMPT_ACTIVE",
        "این تلاش هنوز به پایان نرسیده است.",
      );
  ...
  private normalizeScoring(
    scoring?: Partial<Exam["scoring"]> | null,
  ): Exam["scoring"] {
