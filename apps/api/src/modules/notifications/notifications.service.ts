...
import { decodeCursor, encodeCursor, normalizePageLimit, projectNotification } from "@moshaver/cmb-notifications";
...
export class NotificationsService {
  ...
  async findForUser(userId:string,options:{limit?:number;cursor?:string;unreadOnly?:boolean;category?:string}={}){const limit=normalizePageLimit(options.limit);const qb=this.notifications.createQueryBuilder("n").where("n.userId = :userId",{userId});if(options.unreadOnly)qb.andWhere("n.readAt IS NULL");if(options.category)qb.andWhere("n.category = :category",{category:options.category});if(options.cursor){try{qb.andWhere("(n.createdAt < :createdAt OR (n.createdAt = :createdAt AND n.id < :id))",decodeCursor(options.cursor));}catch{throw new ApiException(400,"INVALID_CURSOR","نشانگر صفحه نامعتبر است.");}}const rows=await qb.orderBy("n.createdAt","DESC").addOrderBy("n.id","DESC").take(limit+1).getMany();const hasMore=rows.length>limit;const items=rows.slice(0,limit);const last=items.at(-1);return{items:items.map(x=>this.publicNotification(x)),unreadCount:await this.getUnreadCount(userId),hasMore,nextCursor:hasMore&&last?encodeCursor({createdAt:last.createdAt,id:last.id}):null};}
  ...
  private publicNotification(n:Notification){return projectNotification(n);}
}
