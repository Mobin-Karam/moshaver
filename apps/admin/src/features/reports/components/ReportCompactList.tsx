      ...
      <table className="w-full min-w-[780px] text-sm">
        <thead className="bg-slate-50 dark:bg-slate-900/30 text-xs text-slate-500">
          <tr>
            ...
            <th className="px-3 text-right">مطالعه</th>
            <th className="px-3 text-right">تست</th>
            <th className="px-3 text-right">دقت</th>
            <th className="px-3 text-right">تمرکز</th>
            <th className="px-3 text-right">انگیزه</th>
            <th className="px-3 text-right">خستگی</th>
            <th className="px-3 text-right">یادداشت</th>
          </tr>
        </thead>
        <tbody>
          {reports.map((row, index) => {
            ...
            return (
              <tr
                key={String(row.id ?? `${date}-${index}`)}
                className="border-t border-slate-100 hover:bg-slate-50 dark:bg-slate-900/30/70"
              >
