...
export function ResourcesPage() {
  ...
  const filtered = useMemo(
    () =>
      (students.data || []).filter((item) =>
        `${item.name} ${item.grade} ${item.major}`
          .toLowerCase()
  ...
  const edit = (item: LearningResource) => {
    setEditing(item.id);
    setForm({
      title: item.title,
  ...
  return (
    <section className="space-y-5">
      <div>
        <p className="text-xs font-bold text-brand">کتابخانه قابل تخصیص</p>
        <h1 className="mt-1 text-2xl font-black">منابع آموزشی دانش‌آموزان</h1>
        <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600 dark:text-slate-300">
          پیوند یا ویدئوی امن را یک‌بار بسازید، چند دانش‌آموز را هم‌زمان انتخاب کنید و وضعیت انتشار
          را کنترل کنید.
        </p>
      </div>
      <Card className="p-4 sm:p-5">
        <form className="grid gap-4" onSubmit={submit}>
          <div className="flex items-center justify-between gap-3">
            <h2 className="font-black">{editing ? "ویرایش منبع" : "منبع تازه"}</h2>
            {editing ? (
              <Button
                type="button"
                variant="ghost"
          ...
          <fieldset className="rounded-2xl border p-3">
            <legend className="px-2 text-sm font-bold">
              دانش‌آموزان دریافت‌کننده ({form.studentIds.length})
            </legend>
              ...
              <Button
                ...
                onClick={() =>
                  setForm({
                    ...form,
                    studentIds: [
                      ...new Set([...form.studentIds, ...filtered.map((item) => item.id)]),
                    ],
                  })
                }
              >
            ...
            <div className="mt-3 grid max-h-64 gap-2 overflow-auto sm:grid-cols-2 lg:grid-cols-3">
              {filtered.map((student) => (
                <label
                  key={student.id}
                  className="flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border px-3 py-2 hover:border-brand/40"
                >
                  <input
                    type="checkbox"
            ...
            {!filtered.length ? (
              <p className="py-5 text-center text-sm text-slate-500">
                دانش‌آموزی مطابق جست‌وجو نیست.
              </p>
            ) : null}
          </fieldset>
          <div className="flex flex-wrap items-center gap-3">
            <Button type="submit" loading={save.isPending} disabled={!form.studentIds.length}>
              <Plus size={16} />
              {editing ? "ذخیره تغییرات" : "ساخت و تخصیص"}
            </Button>
            {save.isError ? (
              <p role="alert" className="text-sm text-rose-700">
                {save.error instanceof Error ? save.error.message : "ذخیره انجام نشد."}
              </p>
            ) : null}
          </div>
        </form>
      ...
      {loading ? (
        <Card className="p-8 text-center" role="status">
          در حال آماده‌سازی منابع…
        </Card>
      ) : null}
      {failed ? (
        <Card className="p-6 text-center text-rose-700" role="alert">
          <p>دریافت اطلاعات انجام نشد.</p>
          <Button
            className="mt-3"
            variant="soft"
      ...
      {!loading && !failed && !resources.data?.length ? (
        <Card className="p-8 text-center">
          <BookOpen className="mx-auto text-brand" />
          <h2 className="mt-3 font-bold">هنوز منبعی ساخته نشده است</h2>
        </Card>
      ) : null}
        ...
        {resources.data?.map((item) => (
          <Card key={item.id} className="p-4">
            ...
            <p className="mt-3 flex items-center gap-2 text-xs text-slate-500">
              <UsersRound size={15} />
              {item.assignments.length} دانش‌آموز
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              {confirmDelete === item.id ? (
                ...
                <Button variant="ghost" onClick={() => setConfirmDelete(item.id)}>
                  <Trash2 size={16} />
                  حذف
                </Button>
              )}
...
function Select({
  ...
  return (
    <select
      className="h-11 w-full rounded-xl border bg-white px-3 text-sm dark:bg-slate-900"
      value={value}
