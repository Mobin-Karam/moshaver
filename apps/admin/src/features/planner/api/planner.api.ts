...
export const getPlans = (
  ...
  search: string,
  filter: TaskFilter,
) => api.get<Plan[]>(plansUrl(studentId, from, to, search, filter));
...
export const createPlan = (studentId: string, body: PlanDraft) =>
  api.post<Plan>("/plans", { ...body, studentId });
export const updatePlan = (id: string, body: Partial<PlanDraft>) =>
  api.patch<Plan>(`/plans/${id}`, body);
...
export const movePlannerTask = (taskId: string, planId: string, start: string, end: string) =>
  api.patch<Plan>(`/tasks/${taskId}`, { planId, start, end, sortOrder: timeToMinutes(start) });
