...
export function QuickReplies({
  ...
  const rendered = useMemo(
    () => items.map((item) => item.split("{name}").join(personName || "")),
    [items, personName],
  );
        ...
        <div className="flex min-w-0 flex-1 gap-2 overflow-x-auto" aria-label="پاسخ‌های سریع">
          {rendered.map((item, index) => (
            <button
              ...
              type="button"
              className="shrink-0 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-600 transition hover:border-teal-300 hover:bg-teal-50 hover:text-teal-800 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-teal-950 dark:hover:text-teal-200"
        ...
        <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-2 dark:border-slate-700 dark:bg-slate-900">
          ...
          <div className="mt-2 grid gap-1">
            {items.map((item, index) => (
              <div
                key={`${item}-${index}`}
                className="flex items-center gap-2 rounded-lg bg-white px-2 py-1.5 text-xs dark:bg-slate-950"
              >
