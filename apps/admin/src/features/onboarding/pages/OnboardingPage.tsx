...
export function OnboardingPage() {
  ...
  const mutation = useMutation({
    mutationFn: ({ id, values }: { id: string; values: AssignmentInput }) =>
      assignStudent(id, values),
    onSuccess: (result) => {
      setCompleted(result);
      void qc.invalidateQueries({ queryKey: ["onboarding", "pending"] });
    },
  });
  ...
  return (
    <section className="space-y-5">
      ...
      <Card className="p-2">
        <div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="روش تخصیص">
          <ModeButton
            active={mode === "AUTO"}
            icon={<WandSparkles />}
            title="تخصیص خودکار"
            description="سریع و متعادل"
          ...
          <ModeButton
            ...
            title="انتخاب دستی"
            description="سازمان و مشاور مشخص"
      ...
      {completed ? (
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-black">تخصیص با موفقیت انجام شد</h2>
              <p className="mt-1 text-sm">
                سازمان: {completed.organization.name} · مشاور: {displayAdvisor(completed.advisor)}
              </p>
      ...
      {!students.isLoading && !students.isError && !students.data?.length ? (
        <Card className="p-8 text-center">
          ...
          <h2 className="mt-3 font-bold">صف ورودی خالی است</h2>
      ...
      <div className="grid gap-4">
        {students.data?.map((student) => {
          ...
          const set = (patch: Partial<ManualChoice>) =>
            setSelection((current) => ({ ...current, [student.id]: { ...value, ...patch } }));
          const eligibleAdvisors =
            advisors.data?.filter((item) =>
              item.assignments.some(
                (assignment) =>
                  assignment.role === "ADVISOR" &&
                  assignment.organizationId === value.organizationId,
              ),
            ) || [];
          ...
          const input: AssignmentInput =
            mode === "AUTO" ? { mode: "AUTO" } : { mode: "MANUAL", ...value };
          return (
            <Card key={student.id} className="overflow-hidden p-0">
              <div className="grid gap-4 p-4 sm:p-5 lg:grid-cols-[minmax(220px,1fr)_minmax(320px,1.5fr)] lg:items-center">
                <div className="flex items-start gap-3">
                  <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-brand/10 font-black text-brand">
                    {student.name.slice(0, 1)}
                  </span>
                  <div className="min-w-0">
                    <h2 className="truncate font-black">{student.name}</h2>
                ...
                <div>
                  {mode === "AUTO" ? (
                    <div className="rounded-2xl border border-brand/15 bg-brand/5 p-4">
                      ...
                      <p className="mt-1 text-xs leading-6 text-slate-500">
                        سازمان فعال و مشاور دارای نقش معتبر با کمترین تعداد دانش‌آموز فعال انتخاب
                        می‌شوند.
                      </p>
                    </div>
                  ) : (
                    <div className="grid gap-3 sm:grid-cols-2">
                      <Field icon={<Building2 />} label="سازمان">
                        <select
                          value={value.organizationId}
                          onChange={(event) =>
                            set({ organizationId: event.target.value, advisorUserId: "" })
                          }
                        >
                      ...
                      <Field icon={<UsersRound />} label="مشاور">
                        <select
                          value={value.advisorUserId}
                          onChange={(event) => set({ advisorUserId: event.target.value })}
                          disabled={!value.organizationId}
                        >
                          <option value="">
                            {value.organizationId && !eligibleAdvisors.length
                              ? "مشاور فعالی موجود نیست"
                              : "انتخاب مشاور"}
                          </option>
                          {eligibleAdvisors.map((item) => (
                            <option key={item.id} value={item.id}>
                              {displayAdvisor(item)}
                            </option>
                          ))}
                  ...
                  {mode === "MANUAL" && manualUnavailable ? (
                    <p className="mt-2 text-xs text-rose-700">
                      فهرست سازمان یا مشاور دریافت نشد؛ دوباره تلاش کنید یا از حالت خودکار استفاده
                      کنید.
                    </p>
                  ) : null}
                  <Button
                    ...
                    disabled={
                      busy ||
                      (mode === "MANUAL" &&
                        (manualUnavailable || !value.organizationId || !value.advisorUserId))
                    }
                    onClick={() => mutation.mutate({ id: student.id, values: input })}
                  >
      ...
      {mutation.isError ? (
        <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700" role="alert">
          {mutation.error instanceof Error ? mutation.error.message : "تخصیص انجام نشد."}
        </p>
      ) : null}
    </section>
  );
...
function ModeButton({
  ...
  return (
    <button
      ...
      aria-checked={active}
      onClick={onClick}
      className={`flex min-h-20 items-center gap-3 rounded-2xl p-3 text-right transition ${active ? "bg-slate-900 text-white shadow-sm dark:bg-brand" : "hover:bg-slate-50 dark:hover:bg-slate-800"}`}
    >
      <span
        className={`grid size-10 shrink-0 place-items-center rounded-xl ${active ? "bg-white/10" : "bg-brand/10 text-brand"}`}
      >
        {icon}
      </span>
      <span>
        <strong className="block text-sm">{title}</strong>
        <small className={`mt-1 block ${active ? "text-white/65" : "text-slate-500"}`}>
          {description}
        </small>
      </span>
    </button>
...
function Field({
  ...
  return (
    <label className="grid gap-1 text-sm font-bold">
      ...
      <span className="[&>select]:h-12 [&>select]:w-full [&>select]:rounded-xl [&>select]:border [&>select]:bg-white [&>select]:px-3 dark:[&>select]:bg-slate-900">
        {children}
      </span>
    </label>
  );
}
function displayAdvisor(user: { username: string; firstName?: string; lastName?: string }) {
