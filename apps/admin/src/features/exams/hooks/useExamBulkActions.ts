...
import { deleteExam, setExamPublished } from "../api/exams.api";
...
export async function runExamBulkAction({
  ...
  const blocked =
    action === "publish"
      ? selected.filter((id) => !exams.find((exam) => exam.id === id)?.delivery?.questionCount)
      : [];
  ...
  const ok = await modal.confirm({
    title: action === "delete" ? `حذف ${ids.length} آزمون؟` : `تغییر وضعیت ${ids.length} آزمون؟`,
  ...
  const results = await Promise.allSettled(
    ids.map((id) =>
      action === "delete" ? deleteExam(id) : setExamPublished(id, action === "publish"),
    ),
  );
  ...
  failed.length
    ? notify(`${failed.length} عملیات ناموفق ماند؛ موارد مربوطه همچنان انتخاب هستند.`, "warning")
