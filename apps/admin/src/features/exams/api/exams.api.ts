...
export function getExams(studentId: string) {
  return api.get<Exam[]>(`/exams?studentId=${encodeURIComponent(studentId)}`);
}

export function getRetryRequests(studentId: string) {
  return api.get<RetryRequest[]>(
    `/exam-attempt-requests?studentId=${encodeURIComponent(studentId)}`,
  );
}

export function createExam(studentId: string, body: ExamDraft) {
