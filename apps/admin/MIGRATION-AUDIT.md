...
1. `npm test` and `npm run build` in `apps/admin`.
...
The v1.6 directory remains a frozen rollback reference during rollout. The production switch should point the admin domain to `apps/admin`; deletion can happen in a later release after monitored acceptance.
