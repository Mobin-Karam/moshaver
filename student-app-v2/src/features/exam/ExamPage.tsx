      ...
      <section className="space-y-4">
        <div className="surface sticky top-[73px] z-10 p-3">
          ...
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-paper">
            <div className="h-full bg-mint" style={{ width: `${((index + 1) / run.quiz.questions.length) * 100}%` }} />
          </div>
          <p className="mt-2 text-xs text-ink/60">سؤال {index + 1} از {run.quiz.questions.length} | بی‌پاسخ: {unansweredCount(ids, selected)}</p>
        </div>
        ...
        <article className="surface p-4">
          ...
          <div className="mt-4 grid gap-2">
            {question.options.map((option, optionIndex) => {
              ...
              return (
                <button key={key} className={`rounded-md border px-3 py-3 text-right text-sm ${active ? 'border-ink bg-ink text-white' : 'border-black/10 bg-white'}`} onClick={() => setSelected((current) => ({ ...current, [question.id]: key }))}>
          ...
          <article key={exam.id} className="surface p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <button className="mt-3 rounded-md bg-ink px-4 py-2 text-sm text-white disabled:opacity-50" disabled={busy || !exam.delivery?.questionCount} onClick={() => void start(exam.id)}>
                  شروع آزمون
                </button>
              </div>
            </div>
