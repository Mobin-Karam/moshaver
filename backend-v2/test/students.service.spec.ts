...
describe("StudentsService parity projections", () => {
  it("summarizes published task completion across the last seven days", async () => {
    ...
    const service = new StudentsService(repository([{ id: "student-1", plans }]) as any, repository() as any, repository() as any);
  ...
  it("returns topic mastery as supported learning data", async () => {
    ...
    const service = new StudentsService(repository([{ id: "student-1", plans: [] }]) as any, repository() as any, mastery as any);
