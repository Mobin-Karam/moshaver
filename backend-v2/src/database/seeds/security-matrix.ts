...
async function main() {
  if (
    ...
    throw new Error(
      "Refusing to create test identities. Use ALLOW_E2E_SEED=true against a disposable non-production database.",
    );
  }
  ...
  await ensureTeacherSubject(
    await manager
      .getRepository(User)
      .findOneByOrFail({ username: "e2e.teacher.a" }),
    subjectA,
    orgA,
  );
  await ensureTeacherSubject(
    await manager
      .getRepository(User)
      .findOneByOrFail({ username: "e2e.teacher.b" }),
    subjectB,
    orgB,
  );
  const platform = await ensureUser(
    "e2e.platform",
  ...
  async function ensureMembershipAndRole(
    ...
    if (!membership)
      membership = await repo.save(
        repo.create({ user, organization, status: MembershipStatus.ACTIVE }),
      );
  ...
  async function ensureTeacherSubject(
    ...
    const repo = manager.getRepository(TeacherSubjectAssignment);
    const found = await repo.findOne({
      where: {
