...
export class LoginThrottleService {
  ...
  private async recordFailure(ip: string, identifier: string) { const now = new Date(); const windowMs = this.config.get<number>("loginAttemptWindowMs", 15 * 60_000); const lockMs = this.config.get<number>("loginLockMs", 15 * 60_000); const max = this.config.get<number>("loginMaxAttempts", 8); for (const key of this.keys(ip, identifier)) { let row = await this.repo.findOne({ where: { key } }); if (!row || now.getTime() - row.windowStartedAt.getTime() > windowMs) row = this.repo.create({ key, attempts: 0, windowStartedAt: now, lockedUntil: null }); row.attempts += 1; if (row.attempts >= max) row.lockedUntil = new Date(now.getTime() + lockMs); await this.repo.save(row); } }
