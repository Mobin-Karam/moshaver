...
export class ExamsController {
  ...
  private context(user: AuthenticatedUser): UserContext {
    return {
      ...user,
  ...
  listCanonical(@CurrentUser() user: AuthenticatedUser) {
  ...
  async updateCanonical(
    ...
    return this.exams.update(id, { ...dto }).then(ok);
  }
  ...
  @Patch("questions/:id")
  @RequireCapabilities("questions.update")
  async updateQuestionCanonical(
    @CurrentUser() user: AuthenticatedUser,
    @Param("id") id: string,
    @Body() dto: CreateQuestionDto,
  ) {
    await this.requireExamScope(
      user,
      await this.exams.examIdForQuestion(id),
      "questions.update",
    );
    return this.exams.updateQuestion(id, { ...dto }).then(ok);
  }
  @Delete("exams/:examId/questions/:id")
  @RequireCapabilities("questions.delete")
    ...
    for (const studentId of dto.studentIds)
      if (
        ...
        throw new ApiException(
          403,
          "STUDENT_FORBIDDEN",
          "به یکی از دانش‌آموزان دسترسی ندارید.",
        );
  ...
  @Get("students/:studentId/exam-attempts/:attemptId")
  @RequireCapabilities("exams.read")
  async staffStudentAttempt(
    @CurrentUser() user: AuthenticatedUser,
    @Param("studentId") studentId: string,
  ...
  private async requireExamScope(
    ...
    if (
      ...
      throw new ApiException(404, "NOT_FOUND", "آزمون یافت نشد.");
  }
  private async requireStudentScope(
    user: AuthenticatedUser,
    id: string,
    capability: string,
  ) {
  ...
  addQuestion(
    @Param("id") id: string,
    @Body()
    dto: CreateQuestionDto & { question?: string; correctOption?: string },
  ) {
    return this.exams.addQuestion(id, dto).then(ok);
  }
  ...
  @Roles(UserRole.ADMIN)
  importQuestions(
    @Body() body: { examId: string; questions: CreateQuestionDto[] },
  ) {
